 ...
 * the mirror remote is read from the `rubato.*` git config namespace.
 */
...
# Installed by Rubato memory-core. Do not edit by hand - regenerated on startup.
 ...
 * `rubato.memoryRepository.url` (letta used `letta.*`). The hook no-ops when the
 ...
 * `RUBATO_MEMORY_PUSH_SYNC=1` runs it in the foreground, which tests use to keep

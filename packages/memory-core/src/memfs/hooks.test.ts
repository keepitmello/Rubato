...
async function createRepo(): Promise<string> {
  ...
  await run(["git", "config", "user.email", "agent@rubato.local"], dir)
  await run(["git", "config", "user.name", "Rubato Agent"], dir)
...
describe("post-commit mirror push", () => {
  ...
  const SYNC = { RUBATO_MEMORY_PUSH_SYNC: "1" }
  ...
  it("#given a configured mirror #when a commit lands on main #then it is pushed and logged", async () => {
    ...
    await run(["git", "config", "--local", "rubato.memoryRepository.url", `file://${mirror}`], dir)
  ...
  it("#given an unreachable mirror #when a commit lands #then the commit succeeds and the failure is logged", async () => {
    ...
    await run(["git", "config", "--local", "rubato.memoryRepository.url", `file://${missing}`], dir)

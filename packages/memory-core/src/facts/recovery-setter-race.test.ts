...
describe("facts conditional mutation primitives", () => {
  test("preserves a foreign replacement injected after hashing an existing tracked path", async () => {
    ...
    expect((await repo.log()).some((commit) => commit.trailers["Rubato-Facts-Batch"] === recovery.batchId)).toBe(false)
  ...
  test("preserves foreign bytes injected after hashing an existing path for deletion", async () => {
    ...
    const receiptsBefore = (await repo.log()).filter((commit) => commit.trailers["Rubato-Facts-Batch"] !== undefined).length
    ...
    expect((await repo.log()).filter((commit) => commit.trailers["Rubato-Facts-Batch"] !== undefined)).toHaveLength(receiptsBefore)
  })
})

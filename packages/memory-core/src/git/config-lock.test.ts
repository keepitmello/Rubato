...
describe("serialized git config", () => {
  ...
  it("#given two concurrent configSet calls #when they mutate one repo #then their git config processes do not overlap", async () => {
    ...
    // when
    await Promise.all([
      repo.configSet("rubato.testOne", "one"),
      repo.configSet("rubato.testTwo", "two"),
    ])

    // then
    expect(exec.maxConfigWrites).toBe(1)
    expect(await repo.configGet("rubato.testOne")).toBe("one")
    expect(await repo.configGet("rubato.testTwo")).toBe("two")
  })
})

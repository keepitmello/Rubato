...
export async function writeWorktreeFile(
  ...
  await assertSafeParents(root, path)
  const temporary = join(dirname(target), `.${basename(target)}.rubato-${process.pid}-${randomUUID()}`)
...
export async function moveWorktreeFile(root: string, path: string): Promise<string | undefined> {
  ...
  try {
    ...
    const moved = join(dirname(target), `.${basename(target)}.rubato-moved-${process.pid}-${randomUUID()}`)
...
export async function reserveMovedWorktreeDeletion(
  ...
  try {
    ...
    await writeFile(join(target, ".rubato-reservation"), marker, "utf8")
  } catch (error) {
  ...
  return {
    verify: async () => {
      ...
      try {
        if (await readFile(join(target, ".rubato-reservation"), "utf8") !== marker) return undefined
      } catch (error) {
        if (errorCode(error) === "ENOENT") return undefined
        throw error
      }
      return {
        finish: async () => {
          const claimed = join(dirname(target), `.${basename(target)}.rubato-reserved-${process.pid}-${randomUUID()}`)
          try {
          ...
          if (owned.isDirectory()) {
            try {
              ownedMarker = await readFile(join(claimed, ".rubato-reservation"), "utf8")
            } catch (error) {

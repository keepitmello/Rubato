...
export class GitMemoryRepo {
  ...
  async init(options: InitializeGitRepoOptions = {}): Promise<string> {
    ...
    await this.ensureIdentity(options.authorName?.trim() || "Rubato Agent")
    ...
    const author: GitCommitAuthor = {
      ...
      authorName: options.authorName?.trim() || "Rubato Agent",
    }
    if (paths.length > 0) {
  ...
  private async ensureIdentity(authorName: string): Promise<void> {
    if ((await this.configGet("rubato.agentId")) !== this.agentId) {
      await this.configSet("rubato.agentId", this.agentId)
    }
    if (!(await this.configGet("user.email"))) {
      await this.configSet("user.email", `${this.agentId}@rubato.local`)
    }

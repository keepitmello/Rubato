...
describe("runMigration", () => {
  ...
  test("#given a user target marker #when a project target is discovered later #then the project target migrates independently", () => {
    // given
    const fileSystem = new MemoryMigrationFileSystem()
    const projectTarget = "/work/project/.rubato/rubato.jsonc"
    fileSystem.files.set(migrationFixture.sourcePath, `{"task":{"default_concurrency":3}}`)
  ...
  test("#given a transformed document rejected by RubatoConfigSchema #when migrating #then target and sources are untouched", () => {
    // given
    const fileSystem = new MemoryMigrationFileSystem()
    ...
    expect(fileSystem.existsSync("/home/alice/.rubato/.migration-journal.json")).toBe(false)
  })
})

...
export function migrationLockPath(env: MigrationEnvironment): string {
  return toPosixPath(join(resolveHomeDir(env), ".rubato", ".migration.lock"))
}
...
export function acquireMigrationLock(input: {
  ...
  input.fileSystem.mkdirSync(toPosixPath(join(resolveHomeDir(input.env), ".rubato")), { recursive: true })

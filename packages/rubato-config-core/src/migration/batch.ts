...
import { prepareTargetReplacement, prepareTargetWrite, targetDocument, writeRubatoMigrationTarget, writePreparedTarget } from "./commit"
...
export function runMigrations(options: RunMigrationsOptions): MigrationBatchRunResult {
  ...
  const writeTarget = options.writeTarget ?? writeRubatoMigrationTarget

...
import { RubatoConfigSchema } from "../schema"
import { updateRubatoConfig } from "../writer"
...
function validateTarget(targetPath: string, document: Readonly<Record<string, unknown>>): void {
  const result = RubatoConfigSchema.safeParse(document)
...
function writerInput(targetPath: string, env: MigrationEnvironment): { readonly projectDir?: string; readonly scope: "project" | "user" } {
  ...
  const userDirectory = toPosixPath(join(homeDir, ".rubato"))
  const fileName = basename(targetPath)
  if (toPosixPath(dirname(targetPath)) === userDirectory && (fileName === "rubato.json" || fileName === "rubato.jsonc")) {
    return { scope: "user" }
  }
  if (basename(dirname(targetPath)) === ".rubato" && (fileName === "rubato.json" || fileName === "rubato.jsonc")) {
    return { projectDir: dirname(dirname(targetPath)), scope: "project" }
  }
  throw new MigrationTransactionError(`Migration target is not a rubato config path: ${targetPath}`)
}
...
export const writeRubatoMigrationTarget: MigrationTargetWriter = (input): void => {

...
import { loadRubatoConfig, type RubatoConfig } from "../index"
...
describe("loadRubatoConfig top-level Senpi configuration characterization", () => {
  test("#given the current top-level-only user config shape #when resolving the senpi view #then category and agent settings are preserved exactly", () => {
    // given
    const root = mkdtempSync(join(tmpdir(), "rubato-config-top-level-senpi-"))
    const homeDir = join(root, "home")
    const cwd = join(homeDir, "project")
    mkdirSync(join(homeDir, ".rubato"), { recursive: true })
    mkdirSync(cwd, { recursive: true })
    writeFileSync(join(homeDir, ".rubato", "rubato.jsonc"), CURRENT_USER_CONFIG)

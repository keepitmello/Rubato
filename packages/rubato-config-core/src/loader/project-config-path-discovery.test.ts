...
import type { RubatoConfigReadFileSystem } from "./types"

function projectConfigPath(directory: string): string {
  return join(directory, ".rubato", "rubato.jsonc")
}

function makeProjectFileSystem(
  configDirectories: readonly string[],
  symlinkedRubatoDirectories: readonly string[] = [],
): RubatoConfigReadFileSystem {
  const configPaths = configDirectories.map(projectConfigPath)
  const existingPaths = new Set([
    ...configDirectories.map((directory) => join(directory, ".rubato")),
    ...configPaths,
    ...symlinkedRubatoDirectories,
  ])
  const symlinkedPaths = new Set(symlinkedRubatoDirectories)
...
describe("findProjectConfigPathsFarthestFirst", () => {
  ...
  test("#given a symlinked project .rubato directory #when resolving project paths #then it is refused", () => {
    // given
    const projectDir = resolve("/opt/work/project")
    const rubatoDir = join(projectDir, ".rubato")
    const fileSystem = makeProjectFileSystem([projectDir], [rubatoDir])
  ...
  test("#given cwd exceeds the maximum walk depth #when resolving project paths #then the walk is bounded", () => {
    ...
    const fileSystem: RubatoConfigReadFileSystem = {
      existsSync: (path: string): boolean => {
        if (path.endsWith(join(".rubato", "rubato.jsonc"))) checkedDirectories.push(dirname(dirname(path)))
        return false
      },
      readFileSync: (): string => "{}",
    }

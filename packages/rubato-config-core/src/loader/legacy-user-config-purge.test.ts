...
/**
 * The user-scope rubato config root is `~/.rubato` and nothing else. Any reappearance of
 * an XDG / APPDATA / `~/.config` user-config branch reintroduces the split-brain
 ...
 * behavioral tests that populate `$XDG_CONFIG_HOME` and `%APPDATA%` and assert
 * neither the loader nor the writer ever uses them:

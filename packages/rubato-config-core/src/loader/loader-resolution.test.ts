...
function makeFixture(): { readonly cwd: string; readonly homeDir: string; readonly projectDir: string } {
  const homeDir = join(mkdtempSync(join(tmpdir(), "rubato-config-loader-resolution-")), "home")
...
describe("loadRubatoConfig resolution", () => {
  test("#given base, harness, profile-base, and profile-harness layers #when loading the active senpi profile #then each layer contributes in precedence order", () => {
    ...
    const result = loadRubatoConfig({
      cwd: fixture.cwd,
      env: { HOME: fixture.homeDir, RUBATO_PROFILE: "opus" },
      harness: "senpi",
      platform: "linux",
    })
  ...
  test("#given user roots and overlapping codex project roots #when loading the codex view #then roots retain their ordered unique union", () => {
    ...
    writeJsonc(
      join(fixture.homeDir, ".rubato", "rubato.jsonc"),
      `{"codegraph":{"excluded_roots":["/tmp/rubato-a","/tmp/rubato-b"]}}`,
    )
    writeJsonc(
      join(fixture.projectDir, ".rubato", "rubato.jsonc"),
      `{"[codex]":{"codegraph":{"excluded_roots":["/tmp/rubato-b","/tmp/rubato-c"]}}}`,
    )
    ...
    // then
    expect(result.diagnostics).toEqual([])
    expect(result.config.codegraph?.excluded_roots).toEqual(["/tmp/rubato-a", "/tmp/rubato-b", "/tmp/rubato-c"])
  })
  ...
  test("#given an unknown activated profile #when loading a harness view #then a profile diagnostic is emitted and no profile overlay is applied", () => {
    ...
    const result = loadRubatoConfig({
      cwd: fixture.cwd,
      env: { HOME: fixture.homeDir, RUBATO_PROFILE: "ghost" },
      harness: "senpi",
      platform: "linux",
    })

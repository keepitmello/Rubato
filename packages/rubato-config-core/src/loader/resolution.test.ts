...
import { resolveRubatoConfigView, resolveRubatoProfileName } from "../index"

describe("resolveRubatoProfileName", () => {
  test("#given an explicit profile and every environment source #when resolving activation #then the explicit option wins", () => {
    // given
    const options = {
      env: {
        OCX_PROFILE: "ocx",
        RUBATO_PROFILE: "rubato",
    ...
    // when
    const profile = resolveRubatoProfileName(options)

    // then
    expect(profile).toBe("explicit")
  })

  test("#given RUBATO_PROFILE and lower-precedence environment sources #when resolving activation #then RUBATO_PROFILE wins", () => {
    // given
    const options = {
      env: {
        OCX_PROFILE: "ocx",
...
describe("resolveRubatoConfigView", () => {
  test("#given base, harness, profile-base, and profile-harness layers #when resolving a harness profile #then later layers win and control keys are removed", () => {
    ...
    // when
    const result = resolveRubatoConfigView({ config, harness: "senpi", profile: "opus" })
  ...
  test("#given an absent activated profile #when resolving a harness view #then a diagnostic is emitted and the no-profile view is used", () => {
    ...
    // when
    const result = resolveRubatoConfigView({ config, harness: "senpi", profile: "ghost" })

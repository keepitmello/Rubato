...
import { RubatoCategoryConfigSchema } from "./category"

describe("RubatoCategoryConfigSchema warn_unavailable", () => {
  test("#given no category warning override #when category parses #then suppression remains absent", () => {
    // given
    const input = {}

    // when
    const parsed = RubatoCategoryConfigSchema.parse(input)
  ...
  test("#given a non-boolean category warning override #when category parses #then validation fails at the field path", () => {
    ...
    // when
    const result = RubatoCategoryConfigSchema.safeParse(input)

...
import { RubatoMemorySettingsSchema } from "./memory"

describe("RubatoMemorySettingsSchema v2 block defaults", () => {
  ...
  test("#given an unknown key inside any v2 block #when parsed #then the strict schema rejects it", () => {
    ...
    // when
    const results = cases.map((input) => RubatoMemorySettingsSchema.safeParse(input))

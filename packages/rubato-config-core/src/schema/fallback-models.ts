...
import { RubatoModelRefObjectSchema, RubatoReasoningSchema, type RubatoModelRefObject } from "./model-ref"
import { normalizeReasoning, splitReasoningSuffix } from "./reasoning-vocabulary"

export const RubatoThinkingConfigSchema = z.object({
  type: z.enum(["enabled", "disabled"]),
  budgetTokens: z.number().optional(),
}).strict()

/** @deprecated Use RubatoReasoningSchema. */
export const RubatoReasoningEffortSchema = RubatoReasoningSchema
...
export function normalizeLegacyModelEntry(entry: Readonly<Record<string, unknown>>): RubatoModelRefObject {
  return RubatoModelRefObjectSchema.parse(normalizeLegacyModelFields(entry))
}

const RubatoLegacyFallbackModelObjectInputSchema = z.object({
  model: z.string(),
...
export const RubatoFallbackModelObjectSchema = z.preprocess(
  (value) => isRecord(value) ? normalizeLegacyModelFields(value) : value,
  RubatoLegacyFallbackModelObjectInputSchema,
)

export const RubatoFallbackModelsSchema = z.union([
  z.string(),
  z.array(z.string()),
  z.array(RubatoFallbackModelObjectSchema),
  z.array(z.union([z.string(), RubatoFallbackModelObjectSchema])),
])

export type RubatoReasoningEffort = z.infer<typeof RubatoReasoningEffortSchema>
export type RubatoFallbackModelObject = z.infer<typeof RubatoFallbackModelObjectSchema>
export type RubatoFallbackModels = z.infer<typeof RubatoFallbackModelsSchema>

...
describe("unified rubato config schema", () => {
  ...
  test("#given unknown root and profile overlay keys #when parsed #then the strict schema rejects both", () => {
    ...
    // when
    const rootResult = RubatoConfigSchema.safeParse(unknownRoot)
    const profileResult = RubatoConfigSchema.safeParse(unknownProfileKey)
  ...
  test("#given the complete legacy codegraph setting set #when parsed #then values parse and legacy defaults are preserved", () => {
    ...
    const config = {
      codegraph: {
        ...
        daemon: false,
        telemetry: true,
        install_dir: "/tmp/rubato-codegraph",
    ...
    // when
    const explicitResult = RubatoConfigSchema.safeParse(config)
    const defaultResult = RubatoConfigSchema.safeParse({ codegraph: {} })

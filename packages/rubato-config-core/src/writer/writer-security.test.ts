...
import { RubatoConfigWriteError, updateRubatoConfig } from "../index"

function makeFixture(): {
  readonly homeDir: string
  readonly projectDir: string
} {
  const root = mkdtempSync(join(tmpdir(), "rubato-config-writer-security-"))
...
describe("updateRubatoConfig filesystem safety", () => {
  ...
  test("#given symlinked project .rubato directory #when editing project config #then global target is rejected without backup", () => {
    // given
    const fixture = makeFixture()
    const targetConfigDir = join(fixture.homeDir, ".rubato")
    const targetConfigPath = join(targetConfigDir, "rubato.jsonc")
    const original = `{"task":{"default_concurrency":8}}\n`
    mkdirSync(targetConfigDir, { recursive: true })
    writeFileSync(targetConfigPath, original)
    symlinkSync(targetConfigDir, join(fixture.projectDir, ".rubato"))

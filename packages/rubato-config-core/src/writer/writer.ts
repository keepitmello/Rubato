...
import { resolveUserRubatoConfigPath } from "../loader"
import {
  DEFAULT_WRITE_FILE_SYSTEM,
  RubatoConfigWriteError,
  type UpdateRubatoConfigOptions,
  type UpdateRubatoConfigResult,
} from "./types"

const EMPTY_RUBATO_CONFIG = `// Rubato configuration
{
}
`
...
function resolveWritePath(options: UpdateRubatoConfigOptions): string {
  ...
  if (options.scope === "user") {
    const jsoncPath = resolveUserRubatoConfigPath(options.env)
    if (fileSystem.existsSync(jsoncPath)) return jsoncPath
    const jsonPath = join(dirname(jsoncPath), "rubato.json")
    return fileSystem.existsSync(jsonPath) ? jsonPath : jsoncPath
  }
  const jsoncPath = join(options.projectDir ?? process.cwd(), ".rubato", "rubato.jsonc")
  if (fileSystem.existsSync(jsoncPath)) return jsoncPath
  const jsonPath = join(dirname(jsoncPath), "rubato.json")
  return fileSystem.existsSync(jsonPath) ? jsonPath : jsoncPath
}
...
function writeAtomically(path: string, content: string, fileSystem: typeof DEFAULT_WRITE_FILE_SYSTEM): void {
  ...
  try {
    ...
    throw new RubatoConfigWriteError(path, "write", error)
  }
}
...
function assertConfigPathIsSafe(path: string, fileSystem: typeof DEFAULT_WRITE_FILE_SYSTEM): void {
  try {
    if (fileSystem.lstatSync(path).isSymbolicLink()) {
      throw new RubatoConfigWriteError(path, "read", new Error("Refusing to edit symlinked rubato config"))
    }
  } catch (error) {
    if (error instanceof RubatoConfigWriteError) throw error
    throw new RubatoConfigWriteError(path, "read", error)
  }
}
...
function assertProjectConfigDirectoryIsSafe(directory: string, fileSystem: typeof DEFAULT_WRITE_FILE_SYSTEM): void {
  try {
    if (fileSystem.lstatSync(directory).isSymbolicLink()) {
      throw new RubatoConfigWriteError(
        directory,
        "read",
        new Error("Refusing to edit config under symlinked project .rubato directory"),
      )
    }
  } catch (error) {
    if (error instanceof RubatoConfigWriteError) throw error
    throw new RubatoConfigWriteError(directory, "read", error)
  }
}
...
function assertJsoncCanBeModified(path: string, content: string): void {
  ...
  throw new RubatoConfigWriteError(path, "parse", new SyntaxError(message))
}

export function updateRubatoConfig(options: UpdateRubatoConfigOptions): UpdateRubatoConfigResult {
  const fileSystem = options.fileSystem ?? DEFAULT_WRITE_FILE_SYSTEM
  const path = resolveWritePath(options)
  const directory = directoryPath(path)
  const existed = fileSystem.existsSync(path)
  let content = EMPTY_RUBATO_CONFIG
  ...
  if (existed) {
    try {
      ...
      if (error instanceof RubatoConfigWriteError) throw error
      throw new RubatoConfigWriteError(path, "backup", error)
    }
  }

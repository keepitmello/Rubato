...
import { loadRubatoConfig, RubatoConfigWriteError, updateRubatoConfig } from "../index"

function makeFixture(): {
  readonly homeDir: string
  readonly projectDir: string
} {
  const root = mkdtempSync(join(tmpdir(), "rubato-config-writer-"))
...
describe("updateRubatoConfig", () => {
  test("#given commented project config #when editing twice #then comments survive backup is created and second write sees first content", () => {
    ...
    const configPath = join(fixture.projectDir, ".rubato", "rubato.jsonc")
...
`,
    ...
    // when
    const first = updateRubatoConfig({
      scope: "project",
    ...
    const second = updateRubatoConfig({
      scope: "project",
      projectDir: fixture.projectDir,
    ...
    expect(backupFiles[0]).toMatch(/rubato\.jsonc\.bak\.\d{4}-\d{2}-\d{2}T/)
  })
...
`,
  ...
  test("#given missing user config #when editing #then file is created with header and parsed value", () => {
    // given
    const fixture = makeFixture()
    const configPath = join(fixture.homeDir, ".rubato", "rubato.jsonc")

    // when
    const result = updateRubatoConfig({
      scope: "user",
    ...
    expect(content).toContain("// Rubato configuration")
    expect(content).toContain(`"default_concurrency": 6`)
  })

  test("#given existing project rubato json #when editing #then writer preserves json path and loaded settings", () => {
    // given
    const fixture = makeFixture()
    const configPath = join(fixture.projectDir, ".rubato", "rubato.json")
    const shadowPath = join(fixture.projectDir, ".rubato", "rubato.jsonc")
    mkdirSync(join(configPath, ".."), { recursive: true })
  ...
  test("#given existing user rubato json #when editing #then writer preserves json path and loaded settings", () => {
    // given
    const fixture = makeFixture()
    const configPath = join(fixture.homeDir, ".rubato", "rubato.json")
    const shadowPath = join(fixture.homeDir, ".rubato", "rubato.jsonc")
    mkdirSync(join(configPath, ".."), { recursive: true })
    writeFileSync(configPath, `{"task":{"default_concurrency":9,"wait":{"max_ms":70000}}}\n`)
  ...
  test("#given writer cannot create temp file #when editing #then typed error surfaces and no partial remains", () => {
    ...
    // then
    expect(run).toThrow(RubatoConfigWriteError)
  ...
  test("#given XDG and APPDATA env vars present #when editing user scope #then the write target never leaves ~/.rubato", () => {
    // given
    const fixture = makeFixture()
    const env = {
    ...
    // when
    const created = updateRubatoConfig({ scope: "user", edits: [{ path: ["task", "default_concurrency"], value: 3 }], env })

    // then
    expect(created.path).toBe(join(fixture.homeDir, ".rubato", "rubato.jsonc"))

    // when an rubato.json already exists it is edited in place, still inside ~/.rubato
    const jsonFixture = makeFixture()
    const jsonPath = join(jsonFixture.homeDir, ".rubato", "rubato.json")
    mkdirSync(join(jsonPath, ".."), { recursive: true })
    writeFileSync(jsonPath, `{"task":{"default_concurrency":9}}\n`)
    const edited = updateRubatoConfig({
      scope: "user",
      edits: [{ path: ["task", "default_concurrency"], value: 4 }],
      env: {
    ...
    expect(dirname(edited.path)).toBe(join(jsonFixture.homeDir, ".rubato"))
  })

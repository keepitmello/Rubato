...
describe("product model catalog", () => {
  test("#given live extras #when admitted #then only curated rows remain unless kept as current", () => {
    ...
    expect(admitProductCatalogItems([
      { provider: "cursor", id: "grok-4.7-high-fast" },
    ]).map((item: { provider: string; id: string }) => `${item.provider}/${item.id}`)).toEqual(["cursor/grok-4.7"])
    expect(
  ...
  test("#given a live Fast-only cursor row #when listed for spawn surfaces #then the picker identity is the available slug", () => {
    ...
    ])).toEqual(["xai/grok-4.7", "cursor/grok-4.7"])
    expect(isProductCatalogSlug("cursor/grok-4.7-high-fast")).toBe(true)
  ...
  test("#given the picker identity #when launching a child #then --model is the live Fast row", () => {
    expect(productCatalogIdentity("cursor/grok-4.7-high-fast")).toBe("cursor/grok-4.7")
    expect(launchProductModel("cursor/grok-4.7")).toBe("cursor/grok-4.7-high-fast")
    expect(launchProductModel("xai/grok-4.7")).toBe("xai/grok-4.7")
    const visible = expandProductCatalogVisibility(new Set(["cursor/grok-4.7-high-fast"]))
    expect(visible.has("cursor/grok-4.7")).toBe(true)
  })
  ...
  test("#given mixed providers #when sorted #then Sol leads and cursor stays after xai", () => {
    ...
    expect(productCatalogLabel({ provider: "cursor", id: "grok-4.7" })).toBe("Grok 4.7 fast")
    ...
    expect(productCatalogLabel({ provider: "xai", id: "grok-4.7" })).toBe("Grok 4.7")

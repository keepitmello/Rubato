...
const STALE_SESSION_PATTERN = /^rubato-agents-(\d+)(?:-([A-Za-z0-9]+))?$/
...
export async function sweepStaleRubatoAgentSessionsWith(deps: SweepDeps): Promise<number> {
...
export async function sweepStaleRubatoAgentSessions(): Promise<number> {
	const deps = await buildRuntimeDeps()
	return sweepStaleRubatoAgentSessionsWith(deps)
}

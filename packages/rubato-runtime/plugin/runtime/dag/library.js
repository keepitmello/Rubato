...
//
// Library dirs, first hit wins: $RUBATO_DAG_LIBRARY (colon-separated), $PWD/.rubato/dags, $HOME/.rubato/dags.
...
function libraryDirs() {
  ...
  const dirs = []
  const configured = env("RUBATO_DAG_LIBRARY")
  if (typeof configured === "string" && configured !== "") {
    const separator = globalThis.process?.platform === "win32" ? ";" : ":"
    dirs.push(...configured.split(separator).filter((dir) => dir !== ""))
  }
  const pwd = env("PWD")
  if (typeof pwd === "string" && pwd !== "") dirs.push(`${pwd}/.rubato/dags`)
  const home = env("HOME")
  if (typeof home === "string" && home !== "") dirs.push(`${home}/.rubato/dags`)
  return dirs
}

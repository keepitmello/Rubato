...
export async function syncVersion(options = {}) {
  ...
  if (typeof version !== "string" || version.length === 0) {
    throw new Error("Cannot sync Rubato runtime plugin version: package.json has no version")
  }
...
if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (!result.ok) {
    console.error(`Rubato runtime plugin version mismatch: expected ${result.version}, found ${result.current}`)
    process.exit(1)
  }
  console.log(`Rubato runtime plugin version ${result.version}${result.changed ? " synced" : " current"}`)
}

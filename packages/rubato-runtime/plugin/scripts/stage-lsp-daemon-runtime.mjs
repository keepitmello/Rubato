...
const manifestName = ".rubato-runtime-manifest.json"
...
export async function stageLspDaemonRuntime(options = {}) {
  ...
  const tempParent = await mkdtemp(join(dirname(targetDist), ".tmp-rubato-lsp-runtime-"))
  ...
  try {
    ...
    if (process.env.RUBATO_RUNTIME_STAGE_FAIL_AFTER_BACKUP === "1") {

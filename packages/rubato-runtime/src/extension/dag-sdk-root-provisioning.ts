...
export const DAG_SDK_ROOT_ENV = "RUBATO_DAG_SDK_ROOT"

export interface DagSdkRootProvisioningOptions {
  // Defaults to the running extension's own ../runtime/dag (extensions/rubato.js layout), falling back
...
export function createDagSdkRootProvisioning(options: DagSdkRootProvisioningOptions = {}): () => void {
  ...
  return () => {
    try {
      ...
      logger?.warn("rubato-runtime dag sdk root provisioning failed", { error })
    }
  }
}

...
describe("dag definition library", () => {
  ...
  beforeEach(async () => {
    ...
    pwdDags = join(fixtureRoot, "pwd", ".rubato", "dags")
    homeDags = join(fixtureRoot, "home", ".rubato", "dags")
    await mkdir(libDir, { recursive: true })
    await mkdir(pwdDags, { recursive: true })
    await mkdir(homeDags, { recursive: true })
    envMap = {
      RUBATO_DAG_LIBRARY: libDir,
  ...
  describe("#given a stored definition in RUBATO_DAG_LIBRARY", () => {
  ...
  describe("#given definitions scattered across the search dirs", () => {
    it("#then RUBATO_DAG_LIBRARY shadows $PWD/.rubato/dags which shadows $HOME/.rubato/dags", async () => {
    ...
    it("#then on win32 the library env splits on semicolons so drive-letter paths survive", async () => {
      ...
      try {
        installGlobals({ ...envMap, RUBATO_DAG_LIBRARY: `C:\\first;${winLib}` })
        const library = await loadLibrary()
    ...
    it("#then a missing name fails listing the searched dirs", async () => {
      ...
      await expect(library.load("ghost")).rejects.toThrow(/\.rubato\/dags/)
    })

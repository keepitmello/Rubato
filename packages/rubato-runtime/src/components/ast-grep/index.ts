  ...
  return {
    ...
    register(pi: SenpiExtensionAPI, ctx: ComponentContext): void {
      if (typeof pi.registerMcpServer !== "function") {
        ctx.logger.info("Rubato ast-grep skipped: Senpi ExtensionAPI does not expose registerMcpServer", {
      ...
      if (entryStat === undefined || !entryStat.isFile()) {
        ctx.logger.warn("Rubato ast-grep skipped: staged MCP runtime is missing", {
          component: AST_GREP_COMPONENT_NAME,
          entry,
        })
        return

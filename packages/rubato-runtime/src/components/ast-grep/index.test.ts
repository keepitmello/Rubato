...
async function makeStagedEntry(): Promise<string> {
  const runtimeDir = await mkdtemp(join(tmpdir(), "rubato-runtime-ast-grep-runtime-"))
...
describe("createAstGrepComponent", () => {
  ...
  it("#given RUBATO_AST_GREP_PROJECT_CWD #when registered #then passes the explicit project cwd through", async () => {
    const entry = await makeStagedEntry()
    const pi = new FakeExtensionAPI()
    const component = createAstGrepComponent({
      env: { RUBATO_AST_GREP_PROJECT_CWD: "/explicit/project" },
    ...
    expect(pi.mcpServers[0]?.config.env).toEqual({ RUBATO_AST_GREP_PROJECT_CWD: "/explicit/project" })
  })

  it("#given the staged entry is absent #when registered #then skips cleanly and warns", async () => {
    const logger = recordingLogger()
    const pi = new FakeExtensionAPI()
    const missingEntry = join(tmpdir(), `missing-rubato-ast-grep-${process.pid}-${Date.now()}`, "cli.js")
    ...
    expect(logger.entries).toEqual([
      {
        level: "warn",
        message: "Rubato ast-grep skipped: staged MCP runtime is missing",

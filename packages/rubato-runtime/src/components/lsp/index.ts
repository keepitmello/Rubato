...
import type { PostEditDiagnosticsOutcome } from "@rubato/lsp-core/post-edit";
...
const LSP_TOOLS_ENABLED_FLAG = "rubato-runtime-lsp-tools-enabled";
const LSP_POST_EDIT_DIAGNOSTICS_ENABLED_FLAG = "rubato-runtime-lsp-post-edit-diagnostics-enabled";
...
function registerLspFlags(pi: SenpiExtensionAPI): void {
	pi.registerFlag(LSP_TOOLS_ENABLED_FLAG, {
		type: "boolean",
		default: true,
		description: "Enable Rubato LSP tools.",
	});
...
export async function handlePostEditDiagnosticsToolResult(
	...
	if (shouldRunPostEditDiagnostics(event)) {
		reportToolHookStatus(ctx, "(Rubato) Checking LSP Diagnostics");
	}

...
describe("rubato-runtime lsp component", () => {
  ...
  it("#given renamed rubato-runtime lsp flags #when diagnostics are disabled #then no LSP tools register", () => {
    // given
    const test = setup()
    test.pi.setFlag("rubato-runtime-lsp-tools-enabled", false)
    ...
    expect(test.pi.flags.map((flag) => flag.name).sort()).toEqual([
      "rubato-runtime-lsp-post-edit-diagnostics-enabled",
      "rubato-runtime-lsp-tools-enabled",
    ])
  })

...
export function resolveSenpiDaemonRuntime(
  ...
  if (hasCliOverride !== hasVersionOverride) {
    throw new Error("RUBATO_LSP_DAEMON_CLI and RUBATO_LSP_DAEMON_VERSION must be set together")
  }
  if (!hasCliOverride || !hasVersionOverride) {
  ...
  if (!isAbsolute(cliOverride)) {
    throw new Error("RUBATO_LSP_DAEMON_CLI must be an absolute path to an existing regular file")
  }
  const stat = statSync(cliOverride, { throwIfNoEntry: false })
  if (!stat) throw new Error("RUBATO_LSP_DAEMON_CLI points to a missing file")
  if (!stat.isFile()) throw new Error("RUBATO_LSP_DAEMON_CLI must point to a regular file")

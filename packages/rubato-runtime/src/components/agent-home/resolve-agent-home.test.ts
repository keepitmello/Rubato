    ...
    describe("#when several prefixes are set", () => {
      test("#then the branded name wins over the legacy names", () => {
        const home = resolveAgentHome({
          env: {
            RUBATO_CODING_AGENT_DIR: "/explicit/rubato",
        ...
        expect(home).toBe(resolve("/explicit/rubato"))
      })
    ...
    describe("#when the branded home holds engine state", () => {
      test("#then the flat branded home is used", () => {
        const home = resolveAgentHome({
          env: {},
          homeDir: HOME,
          exists: (path) => path === join(HOME, ".rubato", AGENT_HOME_SENTINEL),
        })

        expect(home).toBe(join(HOME, ".rubato"))
      })
    })
    ...
    describe("#when no directory is configured", () => {
      test("#then it wins over the legacy flat home", () => {
        const home = resolveAgentHome({
          ...
          homeDir: HOME,
          exists: (path) =>
            path === join(HOME, ".rubato", "agent", AGENT_HOME_SENTINEL) || path === join(HOME, ".rubato", AGENT_HOME_SENTINEL),
        })

        expect(home).toBe(join(HOME, ".rubato", "agent"))
      })
    })
  })
})

...
function fixture(options: { readonly bound?: boolean } = {}): GuardFixture {
  const memoryRoot = mkdtempSync(join(tmpdir(), "rubato-memory-guard-"))
  const workspace = mkdtempSync(join(tmpdir(), "rubato-memory-guard-workspace-"))
...
describe("memory cross-identity guard", () => {
  ...
  test("#given bash contains a forbidden foreign-root literal #when invoked repeatedly in one session #then it warns once and never blocks", async () => {
    ...
    expect(setup.context.logs).toEqual([{
      level: "warn",
      message: "rubato-runtime memory guard advisory: bash command references another memory identity; shell text is not blocked because this soft guard is not a security boundary",

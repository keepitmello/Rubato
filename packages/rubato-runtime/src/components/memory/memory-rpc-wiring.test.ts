...
import { buildIdentityPaths, GitMemoryRepo, type ReservedRun } from "@rubato/memory-core"
...
async function createFixture(sessionId = "session-rpc"): Promise<Fixture> {
  const root = realpathSync.native(await mkdtemp(join(tmpdir(), "rubato-memory-rpc-wiring-")))
  roots.push(root)
  ...
  return {
    ...
    env: { RUBATO_MEMORY_HOME: join(root, "memory") },
    context,
    sessionId,
    identity,

...
async function createFixtureRepo(
  ...
  const root = mkdtempSync(join(tmpdir(), "rubato-memory-status-"))
  roots.push(root)
...
function segmentContext(input: {
  ...
  const root = mkdtempSync(join(tmpdir(), "rubato-memory-segments-"))
  roots.push(root)
...
describe("refreshMemoryStatus", () => {
  ...
  test("#given a repo with no HEAD #when refresh runs #then no footer or advisory appears", async () => {
    const root = mkdtempSync(join(tmpdir(), "rubato-memory-status-empty-"))
    roots.push(root)

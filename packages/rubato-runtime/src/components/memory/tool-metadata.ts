...
export const MEMORY_TOOL_DESCRIPTION = [
  ...
  "",
  "On success the tool returns `Memory <command> committed locally (<sha>).`, or `Memory <command> committed (<sha>); harness will sync after the turn.` when a remote is configured. Commits use the bound Rubato memory identity.",
  "",
  "Examples:",
  "",
  "```python",
...
export const MEMORY_APPLY_PATCH_DESCRIPTION = [
  "Apply a codex-style patch to memory files in the Rubato memory repo, then automatically commit the change. The harness syncs clean committed memory changes after the turn.",
  "",
  ...
  "- Commits with `reason`, authored by the bound Rubato memory identity",

...
export function createDreamTriggerWiring(options: DreamTriggerWiringOptions): DreamTriggerWiring {
  ...
  const launch = (session: DreamTriggerSession, origin: "idle" | "shutdown" | "manual" | "pressure", request: ManualDreamRequest & {
    ...
    fireDream({
      ...
      request,
      now,
      warnLaunchFailure: (error) => options.logger?.warn("rubato-runtime memory dream launch failed", { error: describe(error) }),
    })

  function track(task: () => Promise<void>): void {
    const promise = task()
      .catch((error: unknown) => {
        options.logger?.warn("rubato-runtime memory dream trigger failed", { error: describe(error) })
      })
      .finally(() => {

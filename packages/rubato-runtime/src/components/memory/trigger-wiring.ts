...
import type { ReflectionEvent, ReflectionRequest, ReservationResult, TriggerConfig } from "@rubato/memory-core"
...
export function createReflectionTriggerWiring(options: ReflectionTriggerWiringOptions): ReflectionTriggerWiring {
  ...
  function track(task: () => Promise<void>): void {
    const promise = task()
      .catch((error: unknown) => {
        options.logger?.warn("rubato-runtime memory reflection trigger failed", { error: describe(error) })
      })
      .finally(() => {
  ...
  function launch(request: ReflectionRequest): void {
    try {
      ...
      options.logger?.warn("rubato-runtime memory reflection launch failed", { error: describe(error) })
    }
  }
 ...
 */
export function resolveReflectionTriggerConfig(settings: RubatoMemorySettings, agentName?: string): ResolvedTriggerConfig {

...
import { buildIdentityPaths, GitMemoryRepo, resolveMemoryIdentity } from "@rubato/memory-core"
...
describe("memory pressure dream wiring", () => {
  test("#given committed system memory at soft pressure #when bind refresh computes the advisory estimate #then it attempts one pressure reservation", async () => {
    const root = realpathSync.native(await mkdtemp(join(tmpdir(), "rubato-memory-pressure-dream-wiring-")))
    roots.push(root)
...
describe("memory pressure compile wiring", () => {
  test("#given a bound fixture repo below pressure #when a committed write crosses the threshold #then the next real compile refresh adds pressure metadata without truncating memory", async () => {
    const root = realpathSync.native(await mkdtemp(join(tmpdir(), "rubato-memory-pressure-wiring-")))
    roots.push(root)
...
describe("memory projection config wiring", () => {
  ...
  async function boundPi(memory: Parameters<typeof loadedMemoryConfig>[0], loadConfig?: () => never) {
    const root = realpathSync.native(await mkdtemp(join(tmpdir(), "rubato-memory-projection-wiring-")))
    roots.push(root)
    const paths = buildIdentityPaths(root, IDENTITY)
    const repo = new GitMemoryRepo({ dir: paths.repo, agentId: IDENTITY })
    await repo.init({
...
async function createFixture(): Promise<{
  readonly cwd: string
  readonly env: { readonly RUBATO_MEMORY_HOME: string }
  readonly context: MemoryIdentityContext
  readonly identity: string
  readonly sessionId: string
}> {
  const root = realpathSync.native(await mkdtemp(join(tmpdir(), "rubato-memory-footer-wiring-")))
  roots.push(root)
...
async function reflectionFixture(settings: ReturnType<typeof memorySettings> | undefined): Promise<{
  ...
  const root = realpathSync.native(await mkdtemp(join(tmpdir(), "rubato-memory-wiring-")))
  roots.push(root)
  const identity = createMemoryIdentityContext({
    identity: "agent-test",
  ...
  describe("#given a pending completion and a bound session with a real UI callback", () => {
    describe("#when afterBind drains the identity completion directory", () => {
      test("#then the callback receives the completion notification payload and level", async () => {
        // given
        const root = realpathSync.native(await mkdtemp(join(tmpdir(), "rubato-memory-wiring-notify-")))
        roots.push(root)
        const identity = createMemoryIdentityContext({

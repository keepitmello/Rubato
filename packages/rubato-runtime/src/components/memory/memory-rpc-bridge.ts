import type { GitTreeSizedEntry } from "@rubato/memory-core"
...
export const MEMORY_UPDATED_RPC_EVENT = "rubato.memory.updated"
/** Pull channel: clients request the current snapshot with an `extension_request` for this method. */
export const MEMORY_STATUS_RPC_METHOD = "rubato.memory.status"
...
/**
 * Publishes memory state to RPC clients: a fingerprint-deduped `rubato.memory.updated` push plus an
 * `rubato.memory.status` pull. Snapshots are RPC-only - they are never appended to the transcript -

...
declare const RUBATO_MEMORY_MCP_BUNDLE: string | undefined

function memoryMcpBundleName(): string {
  return typeof RUBATO_MEMORY_MCP_BUNDLE !== "undefined" ? RUBATO_MEMORY_MCP_BUNDLE : "rubato-memory-mcp.js"
}
...
export interface MemoryToolsOptions {
  ...
  /** Invoked once after each successful commit, never on errors. */
...
export function registerMemoryToolSurface(
  ...
  if (options.exposure === "search" && typeof pi.registerMcpServer === "function") {
    pi.registerMcpServer(MEMORY_MCP_SERVER_NAME, {
      ...
      args: [join(dirname(fileURLToPath(import.meta.url)), memoryMcpBundleName())],
      exposure: "search",
      enabled: true,
    })
    return
...
function createMemoryTool(
  ...
  return {
    ...
    execute: async (_toolCallId, params) => {
      ...
      if (context === undefined) return errorResult(`${MEMORY_TOOL_NAME}: ${MEMORY_UNBOUND_MESSAGE}`)
      try {
...
function createMemoryApplyPatchTool(
  ...
  return {
    ...
    promptSnippet: "memory_apply_patch - apply a codex-style patch to Rubato memory files; auto-commits the change",
    promptGuidelines: [
    ...
    execute: async (_toolCallId, params) => {
      const context = resolveContext()
      if (context === undefined) return errorResult(`${MEMORY_APPLY_PATCH_TOOL_NAME}: ${MEMORY_UNBOUND_MESSAGE}`)
      try {
        const { repo, lock, author } = await prepareEngine(context, options)
        const provenance = readToolProvenance(params)
...
function readToolProvenance(value: unknown): MemoryToolProvenance | undefined {
  ...
  if (typeof provenance.sessionId !== "string" || provenance.sessionId.length === 0) return undefined
  return { sessionId: provenance.sessionId }
}

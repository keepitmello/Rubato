...
async function launchDream(
  ...
  const memory = RubatoMemorySettingsSchema.parse({
  ...
  const config: RubatoConfig = { memory, categories: { quick: { model: "rubato-mock/mock-1", reasoning: "high" } } }
  const loaded: SenpiRubatoConfigResult = { config, diagnostics: [], layers: [], sources: [] }

...
export function createMemoryRunSupervisorIc8Harness() {
  ...
  const makeRun = async (mode: "graceful" | "stubborn") => {
    ...
    await writeFile(
      ...
      `${JSON.stringify({
        ...
        env: {
          ...process.env,
          RUBATO_MEMORY_SUPERVISOR_EXIT_PORT: String(port),
        },
  ...
  const launchSupervisor = (
    ...
    const child = spawn(process.execPath, [supervisorPath, runDir], {
      ...
      env: {
        ...process.env,
        RUBATO_MEMORY_SUPERVISOR_ALLOW_TEST_SEAMS: "1",
        RUBATO_MEMORY_SUPERVISOR_PLATFORM: platform,
        RUBATO_MEMORY_SUPERVISOR_CLOCK_PATH: clockPath,
        RUBATO_MEMORY_SUPERVISOR_TASKKILL_COMMAND: JSON.stringify([
          process.execPath,
          taskkillFixture,
        ]),
        RUBATO_MEMORY_SUPERVISOR_TASKKILL_RUN_DIR: runDir,
        ...(process.platform === "win32" && platform === "posix"
          ? {
              RUBATO_MEMORY_SUPERVISOR_POSIX_SIGNAL_COMMAND: JSON.stringify([
                process.execPath,
                taskkillFixture,
              ]),
            }

...
function testSeamsEnabled(): boolean {
  return process.env.RUBATO_MEMORY_SUPERVISOR_ALLOW_TEST_SEAMS === "1"
}

export function getSupervisorRuntimePlatform(): SupervisorRuntimePlatform {
  if (testSeamsEnabled()) {
    const injected = process.env.RUBATO_MEMORY_SUPERVISOR_PLATFORM
...
export function readSupervisorClockNow(): number {
  const clockDir = testSeamsEnabled() ? process.env.RUBATO_MEMORY_SUPERVISOR_CLOCK_PATH : undefined
...
function recordTestTermination(action: string, targetPid: number): void {
  ...
  const runDir = process.env.RUBATO_MEMORY_SUPERVISOR_TASKKILL_RUN_DIR
...
export function signalSupervisorProcessGroup(pid: number, signal: NodeJS.Signals): void {
  ...
  const injected = testCommand("RUBATO_MEMORY_SUPERVISOR_POSIX_SIGNAL_COMMAND")
...
function taskkillTree(pid: number, synchronous: boolean): void {
  const command = testCommand("RUBATO_MEMORY_SUPERVISOR_TASKKILL_COMMAND") ?? ["taskkill"]

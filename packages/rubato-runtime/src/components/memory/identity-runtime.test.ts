...
describe("memory identity runtime", () => {
  test.skipIf(process.platform !== "darwin" && process.platform !== "linux")(
    ...
    async () => {
      // given
      const root = await mkdtemp(join(tmpdir(), "rubato-memory-identity-runtime-"))
      roots.push(root)
...
async function renderReflectionSandboxAgentDir(envShape: {
  ...
  const root = await mkdtemp(join(tmpdir(), "rubato-memory-agent-dir-"))
  roots.push(root)
  ...
  // its ambient ~/.rubato/settings.json sentinel) and the seeded sentinel forces the flat `~/.rubato`
  ...
  await mkdir(join(fakeHome, ".rubato"), { recursive: true })
  const sentinelPath = join(fakeHome, ".rubato", "settings.json")
  const injectedEnv: Record<string, string | undefined> = {}
  if (envShape.rubato !== undefined) injectedEnv.RUBATO_CODING_AGENT_DIR = envShape.rubato
  if (envShape.senpi !== undefined) injectedEnv.SENPI_CODING_AGENT_DIR = envShape.senpi
  const snapshot = {
    PATH: process.env.PATH,
    RUBATO_CODING_AGENT_DIR: process.env.RUBATO_CODING_AGENT_DIR,
  ...
  delete process.env.RUBATO_CODING_AGENT_DIR
...
describe("memory identity runtime agent-dir resolution", () => {
  test.skipIf(process.platform !== "darwin" && process.platform !== "linux")(
    "#given a rendered reflection sandbox #when RUBATO_CODING_AGENT_DIR is set #then the agent-dir runtimeWrites grant equals the resolved agent dir",
    async () => {
      // given
      const rubatoAgentDir = await mkdtemp(join(tmpdir(), "rubato-agent-dir-"))
      roots.push(rubatoAgentDir)
      // when
      const { renderedArgs } = await renderReflectionSandboxAgentDir({ rubato: rubatoAgentDir })
      // then: assert the LITERAL injected dir. The stale expression ignores
      // RUBATO_CODING_AGENT_DIR, so it can never grant this dir on any host.
      expect(sandboxGrantsAgentDir(renderedArgs, realpathSync(rubatoAgentDir))).toBe(true)
    },
    30_000,
  )
  ...
  test.skipIf(process.platform !== "darwin" && process.platform !== "linux")(
    ...
    async () => {
      // given
      const senpiAgentDir = await mkdtemp(join(tmpdir(), "rubato-runtime-agent-dir-"))
  ...
  test.skipIf(process.platform !== "darwin" && process.platform !== "linux")(
    ...
    async () => {
      // when: RUBATO_, SENPI_ and PI_ unset; the seeded sentinel forces the flat `~/.rubato` branch.
      const { fakeHome, renderedArgs } = await renderReflectionSandboxAgentDir({})
      // then: assert the LITERAL join(fakeHome, ".rubato"). The stale expression resolves
      ...
      expect(sandboxGrantsAgentDir(renderedArgs, realpathSync(join(fakeHome, ".rubato")))).toBe(true)
    },
    30_000,
  )
})

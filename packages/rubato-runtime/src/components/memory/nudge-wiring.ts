...
import type { GitMemoryRepo, MemoryToolProvenance } from "@rubato/memory-core"
...
export const ACCEPTED_TURNS_ENTRY_TYPE = "rubato-memory:accepted-turns"
  ...
  return {
    ...
    async nudgeTurns(repo, sessionId, identity): Promise<number | undefined> {
      ...
      const lastSave = history.find((commit) =>
        commit.trailers["Rubato-Writer"] === "memory-tool"
        && commit.trailers["Rubato-Session"] === sessionId
        && parseTurn(commit.trailers["Rubato-Turn"]) !== undefined
      )

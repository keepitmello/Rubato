...
import * as dagEngine from "@rubato/senpi-task/dag"
...
function fakeUi(widgetCalls: Array<string[] | undefined>): CapturedUi {
  return {
    ...
    setStatus: () => undefined,
    setWidget: (key, rows) => {
      if (key === "rubato-dag") widgetCalls.push(rows)
    },
...
async function runtimeFixture(options: RuntimeFixtureOptions = {}): Promise<RuntimeFixture> {
  const project = options.project ?? fs.mkdtempSync(join(tmpdir(), "rubato-dag-lifecycle-"))
  ...
  const engine = composeTaskEngine({
    pi,
    rubatoConfig: loadRubatoConfig({ cwd: project }).config,
    cwd: project,
    sharedParentTools: () => [],
    runnerFactories,
...
class ReferenceDagViewer {
  ...
  async catchUp(): Promise<DagRpcSubscribeHandshake> {
    this.rpc.listen("rubato.dag.event", (data) => {
    ...
    const handshake = expectOk<DagRpcSubscribeHandshake>(
      await this.rpc.call("rubato.dag.subscribe", { runId: this.runId, limit: 2 }),
    )
    this.applyPage(handshake.page.events)
    let page = handshake.page
    while (page.hasMore) {
      page = expectOk<DagRpcHistoryPage>(await this.rpc.call("rubato.dag.history", {
        runId: this.runId,
  ...
  private async applyLive(event: DagRpcEvent): Promise<void> {
    ...
    if (event.seq > this.lastSeq() + 1) {
      const page = expectOk<DagRpcHistoryPage>(await this.rpc.call("rubato.dag.history", {
        runId: this.runId,
...
describe("assembled DAG lifecycle end to end", () => {
  ...
  test("#given a pending adapter run #when shutdown persists a pause and a fresh adapter attaches #then it claims, resumes, and completes the run", async () => {
    // given
    const project = fs.mkdtempSync(join(tmpdir(), "rubato-dag-lifecycle-restart-"))
  ...
  test("#given bursty adapter mutations with an identical final state #when the snapshot window flushes #then rubato.dag.updated emits that state once", async () => {
    // given
    const fixture = await runtimeFixture()
    ...
    const firstWindow = fixture.rpc.events.filter((event) => event.name === "rubato.dag.updated")
    fixture.runtime.sync()
    fixture.runtime.sync()
    fixture.bridgeTimers.flush(50)
    const secondWindow = fixture.rpc.events.filter((event) => event.name === "rubato.dag.updated")

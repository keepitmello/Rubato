...
function rpcBus(options: { readonly dropLiveSeq: number; readonly duplicateLiveSeq: number }) {
  ...
  const publish = (name: string, data: unknown): void => {
    ...
    if (name === "rubato.dag.event" && event.seq === options.dropLiveSeq) return
    const deliveries = name === "rubato.dag.event" && event.seq === options.duplicateLiveSeq ? 2 : 1
...
class ReferenceDagViewer {
  ...
  async catchUp(): Promise<DagRpcSubscribeHandshake> {
    this.rpc.listen("rubato.dag.event", (value) => {
    ...
    this.rpc.listen("rubato.dag.activity", (value) => {
    ...
    const handshake = expectOk<DagRpcSubscribeHandshake>(
      await this.rpc.call("rubato.dag.subscribe", { runId: this.runId, limit: this.pageLimit }),
    )
  ...
  private async history(sinceSeq: number, throughSeq: number): Promise<DagRpcHistoryPage> {
    ...
    return expectOk<DagRpcHistoryPage>(await this.rpc.call("rubato.dag.history", {
      runId: this.runId,
      sinceSeq,
      throughSeq,
...
describe("dag viewer reference consumer", () => {
  it("#given a 200-event live run #when catch-up races live delivery #then application is gap-free and duplicate-free", async () => {
    // given
    const projectDir = fs.mkdtempSync(join(tmpdir(), "rubato-dag-viewer-"))
    ...
    // then
    expect(bus.operations.indexOf("listen:rubato.dag.event")).toBeLessThan(
      bus.operations.indexOf("call:rubato.dag.subscribe"),
    )
    ...
    bus.publish("rubato.dag.event", viewer.applied.at(-1))
    await viewer.idle()

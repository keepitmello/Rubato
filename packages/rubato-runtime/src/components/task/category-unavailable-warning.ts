...
function warningSuppressed(config: RubatoConfig, settings: RubatoTaskSettings, category: string): boolean {
...
export function createCategoryUnavailableWarningPlanner(deps: CategoryUnavailableWarningDeps): ChildPlanner {
  ...
  return (spec): PlanResolution => {
    ...
    if (hasUserModel(deps.rubatoConfig, category)) return resolution
    if (warningSuppressed(deps.rubatoConfig, deps.settings, category)) return resolution

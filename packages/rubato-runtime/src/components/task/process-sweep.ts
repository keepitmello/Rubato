...
} from "@rubato/utils/process-sweep"

import type { ComponentContext, SenpiExtensionAPI } from "../../extension/types"
import { resolveSenpiDaemonRuntime } from "../lsp/daemon-runtime"

// Unconditional Rubato process hygiene for the senpi adapter (T16): fired on
...
const DEFAULT_FAMILY_SWEEPS: RubatoFamilySweeps = {
...
export interface SessionStartProcessSweepOptions {
  ...
  readonly sweep?: RubatoFamilySweep
  readonly resolveDaemonVersion?: (env: Record<string, string | undefined>) => string
  readonly familySweeps?: RubatoFamilySweeps
}
...
export function wireSessionStartProcessSweep(
  ...
  const sweep = options.sweep ?? (() => {
    return sweepRubatoFamiliesBestEffort(ctx, () => resolveDaemonVersion(env), options.familySweeps)
  })
...
function runSweepBestEffort(sweep: RubatoFamilySweep, ctx: ComponentContext): void {
  try {
    void Promise.resolve()
      .then(() => sweep())
      .catch((error: unknown) => {
        ctx.logger.warn("rubato-runtime process sweep failed", {
          error: error instanceof Error ? error.message : String(error),
        })
      })
  } catch (error) {
    ctx.logger.warn("rubato-runtime process sweep failed to start", {
      error: error instanceof Error ? error.message : String(error),
    })
  }
}

async function sweepRubatoFamiliesBestEffort(
  ctx: ComponentContext,
  resolveLspDaemonVersion: () => string,
  sweeps: RubatoFamilySweeps = DEFAULT_FAMILY_SWEEPS,
): Promise<void> {

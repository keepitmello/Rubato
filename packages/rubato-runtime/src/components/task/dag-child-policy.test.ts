...
import { buildRpcSpawn } from "@rubato/senpi-task"

const tempDirs: string[] = []
const RUBATO_EXTENSION = "/opt/rubato/plugin/extensions/rubato.js"
const PROVIDER_EXTENSION = "/tmp/mock-provider.ts"

function rpcSpec(owner?: object) {
  const stateRoot = mkdtempSync(join(tmpdir(), "rubato-runtime-dag-rpc-policy-"))
  ...
  return {
    ...
    state_dir: stateDir,
    prompt: "do the work",
    extensions: [RUBATO_EXTENSION, PROVIDER_EXTENSION],
  }
}
...
describe("DAG child rpc extension policy", () => {
  test("#given DAG-owned and ordinary rpc tasks #when child spawn args are built #then only the DAG child drops rubato-runtime", () => {
    // given
    ...
    // then
    expect(dagArgs).not.toContain(RUBATO_EXTENSION)
    expect(dagArgs).toContain(PROVIDER_EXTENSION)
    expect(plainArgs).toContain(RUBATO_EXTENSION)

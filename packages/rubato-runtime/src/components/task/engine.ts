...
export function composeTaskEngine(deps: ComposeTaskEngineDeps): TaskEngine {
  const settings: RubatoTaskSettings = deps.rubatoConfig.task ?? RubatoTaskSettingsSchema.parse({})
  ...
  const memberLiveness = createTeamMemberLivenessNotifier({
    ...
    wasDelivered: (record) => {
      try {
        ...
        log("rubato-runtime team liveness marker read failed", {
    ...
    markDelivered: (record) => {
      try {
        ...
        log("rubato-runtime team liveness marker write failed", {
          taskId: record.task_id,
          error: error instanceof Error ? error.message : String(error),
        })
      }
    ...
    onError: (error) => {
      log("rubato-runtime team liveness delivery failed", {
        error: error instanceof Error ? error.message : String(error),
      })
    },
  })
  const notifyOwnedMemberLiveness = createOwnedMemberLivenessNotifier({
    ...
    onError: (error, record) => {
      log("rubato-runtime team liveness ownership check failed", {
        taskId: record.task_id,
        error: error instanceof Error ? error.message : String(error),
      })
    },
  })
  const agents = resolveTaskAgents(deps.rubatoConfig)
  ...
  const appendTaskEvent = (taskId: string, event: PersistedTaskEvent): void => {
    try {
      ...
      log("rubato-runtime task event append failed", {
        taskId,

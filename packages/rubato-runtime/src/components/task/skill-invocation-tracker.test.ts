...
describe("createSkillInvocationTracker - plan-artifact channel", () => {
  ...
  test("#given a write into a worktree .rubato/plans file #when the tool result arrives #then the session has a plan artifact", async () => {
    // given
    const pi = new FakeExtensionAPI()
    const tracker = createSkillInvocationTracker(pi)

    // when
    await pi.dispatch("tool_result", toolResult("write", { path: "/repo/.local-ignore/worktrees/wt1/.rubato/plans/feature.md" }), CTX_A)
  ...
  test("#given an edit of a relative .rubato/plans path #when the tool result arrives #then the artifact is recorded", async () => {
    // given
    const pi = new FakeExtensionAPI()
    const tracker = createSkillInvocationTracker(pi)

    // when
    await pi.dispatch("tool_result", toolResult("edit", { path: ".rubato/plans/refactor.md" }), CTX_A)
  ...
  test("#given a read of a plan file written by a planning child #when the tool result arrives #then the artifact is recorded", async () => {
    ...
    // when
    await pi.dispatch("tool_result", toolResult("read", { path: "/other/checkout/.rubato/plans/child-authored.md" }), CTX_A)

    // then
    expect(tracker.stateFor("sess-a").hasPlanArtifact()).toBe(true)
  })

  test("#given an apply_patch whose patch body touches .rubato/plans #when the tool result arrives #then the artifact is recorded", async () => {
    // given
    const pi = new FakeExtensionAPI()
    const tracker = createSkillInvocationTracker(pi)
  ...
  test("#given non-plan writes and failed plan writes #when the tool results arrive #then no artifact is recorded", async () => {
    ...
    await pi.dispatch("tool_result", toolResult("write", { path: ".rubato/plans/broken.md" }, true), CTX_A)
    await pi.dispatch("tool_result", toolResult("grep", { path: ".rubato/plans/scan.md" }), CTX_A)
...
describe("createSkillInvocationTracker - expanded skill block channel", () => {
  ...
  test("#given an expanded ulw-plan skill block #when the input arrives #then it counts as invocation and user request", async () => {
    ...
    await pi.dispatch(
      ...
      {
        type: "input",
        text: '<skill name="ulw-plan" location="/Users/u/.bun/install/global/node_modules/rubato-ai/plugin/skills/ulw-plan/SKILL.md"> References are relative to the skill dir.',
        source: "interactive",
      },
      CTX_A,
    )

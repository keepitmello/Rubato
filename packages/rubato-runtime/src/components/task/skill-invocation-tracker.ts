...
// - plan artifact: a successful read/write/edit on a .rubato/plans/*.md path at ANY root (worktrees
...
const PLAN_ARTIFACT_PATH_PATTERN = /(^|[\\/])\.rubato[\\/]plans[\\/][^\\/]+\.md$/i
const PLAN_ARTIFACT_PATCH_PATTERN = /\.rubato[\\/]plans[\\/][^\s"'`]+\.md/i
...
// read/write/edit, or every distinct .rubato/plans/*.md match in an apply_patch body (each counted

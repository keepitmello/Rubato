...
export function createCategoryConfigGenerations(mask: MaskCategoryModel = identityMask): CategoryConfigGenerations {
  ...
  return {
    observe: ({ rubatoConfig, registry }) => {
      const categories = canonicalCategories(rubatoConfig, registry, mask)
      const userNames = userCategoryNames(rubatoConfig)
      const next = {
        categories,
...
export function createGenerationObservingPlanner(input: {
  ...
  const { planner, rubatoConfig, resolveRegistry, generations } = input
  return (spec) => {
    const resolution = planner(spec)
    if (resolution.kind !== "resolved") return resolution
    const registry = resolveRegistry()
    if (registry !== undefined) generations.observe({ rubatoConfig, registry })
    return resolution
  }
}
...
function userCategoryNames(rubatoConfig: RubatoConfig): readonly string[] {
  return Object.keys(rubatoConfig.categories ?? {})
}
...
function canonicalCategories(
  ...
  const entries = BUILTIN_CATEGORY_NAMES.map((name) => [name, encodeCategory(name, rubatoConfig, registry, mask)] as const)
...
function encodeCategory(
  ...
  const resolution = resolveCategory<SenpiModelPort>(name, rubatoConfig, registry)

...
import { RubatoTaskSettingsSchema, type RubatoTaskSettings } from "@rubato/config-core"
import { saveRuntimeState } from "@rubato/team-core/team-state-store"
import type { RuntimeState } from "@rubato/team-core/types"
import {
...
function createHarness(options: {
  ...
  const emitter = createResumptionChannelEmitter({
    ...
    stateDir: { project_dir: "/tmp/rubato-resumption-channel-test" },
    settings: {} as RubatoTaskSettings,
...
describe("createResumptionChannelEmitter", () => {
  ...
  it("#given a foreground owned team member resolved through the real ownership helper #when the session snapshot is emitted #then the member counts as active", async () => {
    ...
    const project = mkdtempSync(join(tmpdir(), "rubato-resumption-channel-binding-"))
    tempRoots.push(project)
    const settings = RubatoTaskSettingsSchema.parse({})

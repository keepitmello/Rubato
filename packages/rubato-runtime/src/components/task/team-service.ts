...
import { listActiveTeams, loadRuntimeState } from "@rubato/team-core/team-state-store"
...
export function createTeamService(deps: TeamServiceDeps): TeamToolsService {
  ...
  const service: TeamToolsService = {
    async createTeam(input) {
      ...
      const { spec, source } = await resolveTeamSpec(input, ports, deps.cwd, rubatoTeams)

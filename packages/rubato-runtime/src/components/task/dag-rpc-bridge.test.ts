...
describe("dag rpc bridge", () => {
  describe("#given a journal that fans out post-durability", () => {
    it("#when events arrive #then every event is emitted exactly once on rubato.dag.event in seq order", () => {
      // given
      ...
      // then
      const events = emittedNames(emitted, "rubato.dag.event")
    ...
    it("#when a run starts mid-session and sync runs #then its events reach the ledger too", () => {
      ...
      // then
      const events = emittedNames(emitted, "rubato.dag.event").map((entry) => entry.data as DagBridgeRunEvent)
    ...
    it("#when the same seq is redelivered #then it is emitted once", () => {
      ...
      // then
      expect(emittedNames(emitted, "rubato.dag.event")).toHaveLength(1)
    })
  })

  describe("#given a session that owns a live run", () => {
    it("#when the heartbeat interval elapses #then rubato.dag.heartbeat carries every live run head seq", () => {
      // given
      const source = fakeRun("dag_1", "running")
      ...
      // then
      const beats = emittedNames(emitted, "rubato.dag.heartbeat")
    ...
    it("#when the run reaches a terminal status #then the heartbeat stops and no timer is left pending", () => {
      ...
      bridge.attach()
      timers.advance(DAG_DEFAULT_HEARTBEAT_MS)
      expect(emittedNames(emitted, "rubato.dag.heartbeat")).toHaveLength(1)
    ...
    it("#when only terminal runs exist at attach #then no heartbeat is ever emitted", () => {
      ...
      // then
      expect(emittedNames(emitted, "rubato.dag.heartbeat")).toHaveLength(0)
      expect(timers.pending()).toBe(0)
    })
  ...
  describe("#given unsequenced activity telemetry", () => {
    it("#when a node bursts activity #then it coalesces to the latest payload on rubato.dag.activity only", () => {
      // given
      const source = fakeRun("dag_1", "running")
      const { bridge, emitted, timers } = wire([source.run])
      bridge.attach()
      ...
      // then
      const activity = emittedNames(emitted, "rubato.dag.activity")
      expect(activity).toHaveLength(1)
      expect((activity[0]?.data as DagBridgeActivityEvent).activity).toBe("running tests")
      expect(emittedNames(emitted, "rubato.dag.event")).toHaveLength(0)
    })
    ...
    it("#when two nodes are active #then coalescing is per node", () => {
      ...
      // then
      const activity = emittedNames(emitted, "rubato.dag.activity").map(
    ...
    it("#when activity is published after detach #then nothing is emitted and no timer survives", () => {
      ...
      // then
      expect(emittedNames(emitted, "rubato.dag.activity")).toHaveLength(0)
      expect(timers.pending()).toBe(0)
    })
  })

  describe("#given the rubato.dag.updated snapshot channel", () => {
    it("#when the store mutates twice with identical state #then only one snapshot is emitted", () => {
      ...
      expect(emittedNames(emitted, "rubato.dag.updated")).toHaveLength(1)
    ...
    it("#when a real mutation lands #then a fresh snapshot reflecting it is emitted", () => {
      ...
      // then
      const updates = emittedNames(emitted, "rubato.dag.updated")
    ...
    it("#when the payload is inspected #then it is snake_case and carries parent_session_id", () => {
      ...
      // then
      const payload = emittedNames(emitted, "rubato.dag.updated")[0]?.data
    ...
    it("#when more runs exist than the cap #then truncated_runs reports the overflow", () => {
      ...
      // then
      const payload = emittedNames(emitted, "rubato.dag.updated")[0]?.data as {
        runs: { run_id: string }[]
    ...
    it("#when the run count is at the cap #then truncated_runs is absent", () => {
      ...
      // then
      const payload = emittedNames(emitted, "rubato.dag.updated")[0]?.data as { truncated_runs?: number }
    ...
    it("#when the session has no parent session id #then nothing is emitted on the snapshot channel", () => {
      ...
      // then
      expect(emittedNames(emitted, "rubato.dag.updated")).toHaveLength(0)
    })
    ...
    it("#when the bridge detaches #then a pending snapshot never lands and the fingerprint resets", () => {
      ...
      bridge.attach()
      timers.advance(DAG_SNAPSHOT_DEBOUNCE_MS)
      expect(emittedNames(emitted, "rubato.dag.updated")).toHaveLength(2)
    })
    ...
    it("#when the granular ledger emits #then the snapshot channel carries no per-event deltas", () => {
      ...
      // then
      expect(emittedNames(emitted, "rubato.dag.event")).toHaveLength(2)
      expect(emittedNames(emitted, "rubato.dag.updated")).toHaveLength(1)
    })
  })
  ...
  describe("#given a configured heartbeat interval", () => {
    it("#when task.dag.heartbeat_ms overrides the default #then beats follow the configured period", () => {
      ...
      // then
      expect(emittedNames(emitted, "rubato.dag.heartbeat")).toHaveLength(2)
    })
  })
})

...
import { loadRubatoConfig, RubatoTaskSettingsSchema } from "@rubato/config-core"
import {
...
async function seedPendingRun(cwd: string, runId: DagRunId, sessionId: string): Promise<void> {
  await createDagManager({ store: dagStore(cwd), newRunId: () => runId }).start({
    ...
    definition: {
      ...
      nodes: [{ id: "resume", prompt: "resume", subagent_type: "explore", model: "rubato-mock/mock-1" }],
    },
  })
}
...
function fakeUi(widgetRows: string[][]): CapturedUi {
  return {
    ...
    setStatus: () => undefined,
    setWidget: (key, rows) => {
      if (key === "rubato-dag" && rows !== undefined) widgetRows.push(rows)
    },
...
describe("assembled DAG runtime", () => {
  test("#given a running DAG node #when its assembled child emits progress #then RPC and the live widget share one activity subscription that tears down", async () => {
    // given
    const cwd = fs.mkdtempSync(join(tmpdir(), "rubato-runtime-dag-activity-"))
    ...
    const pi = Object.assign(new FakeExtensionAPI(), {
      rpc: {
        emit: (name: string, data: unknown) => {
          ...
          if (name === "rubato.dag.event" && typeof data === "object" && data !== null &&
    ...
    const started = await runtime.manager.start({
      ...
      definition: {
        ...
        nodes: [{ id: "active", prompt: "work", subagent_type: "explore", model: "rubato-mock/mock-1" }],
      },
    })
    ...
    // then
    expect(rpcEvents.filter((event) => event.name === "rubato.dag.activity")).toEqual([
      {
        name: "rubato.dag.activity",
  ...
  test("#given an unknown node skill #when the assembled runtime creates the run #then snapshot records missing_skill and dispatch keeps the prompt", async () => {
    // given
    const cwd = fs.mkdtempSync(join(tmpdir(), "rubato-runtime-dag-missing-skill-"))
    cleanupRoots.push(cwd)
    const runner = new ScriptedRunner()
  ...
  test("#given a terminal wake buffered during compaction #when the assembled runtime attaches on session start #then it redelivers exactly once", async () => {
    // given
    const cwd = fs.mkdtempSync(join(tmpdir(), "rubato-runtime-dag-wake-redelivery-"))
    cleanupRoots.push(cwd)
    const runner = new ScriptedRunner()
    ...
    const started = await runtime.manager.start({
      ...
      definition: {
        ...
        nodes: [{ id: "complete", prompt: "complete", subagent_type: "explore", model: "rubato-mock/mock-1" }],
      },
    })
    await within(runner.whenStarted(1))
    runner.handles[0]?.settle("done")
  ...
  test("#given a live task component runtime #when a two-node dag runs #then scheduler, artifacts, rpc, widget, and one wake all observe the same run", async () => {
    // given
    const cwd = fs.mkdtempSync(join(tmpdir(), "rubato-runtime-dag-runtime-"))
    cleanupRoots.push(cwd)
    const runner = new ScriptedRunner()
    ...
    const started = await runDagTool(
      ...
      },
      {
        ...
        definition: {
          ...
          nodes: [
            { id: "plan", prompt: "plan", subagent_type: "explore", model: "rubato-mock/mock-1" },
            { id: "build", prompt: "build", subagent_type: "explore", model: "rubato-mock/mock-1", dependsOn: ["plan"] },
          ],
        },
      },
    )
    ...
    expect(rpcEvents.some((event) => event.name === "rubato.dag.event")).toBe(true)
    expect(rpcEvents.some((event) => event.name === "rubato.dag.updated")).toBe(true)
    ...
    expect(rpcHandlers.has("rubato.dag.snapshot")).toBe(true)
    ...
    const cancellable = await runDagTool(
      ...
      },
      {
        ...
        definition: {
          ...
          nodes: [
            { id: "hold", prompt: "hold", subagent_type: "explore", model: "rubato-mock/mock-1" },
            { id: "never", prompt: "never", subagent_type: "explore", model: "rubato-mock/mock-1", dependsOn: ["hold"] },
          ],
        },
      },
    )
    ...
    const copyFailure = await runDagTool(
      ...
      },
      {
        ...
        definition: {
          ...
          nodes: [{ id: "copy", prompt: "copy", subagent_type: "explore", model: "rubato-mock/mock-1" }],
        },
      },
    )
  ...
  test("#given a live-shaped in-process child whose abort rejects #when the assembled runtime cancels #then no unhandled rejection escapes and a later run still completes", async () => {
    // given
    const cwd = fs.mkdtempSync(join(tmpdir(), "rubato-runtime-dag-abort-boundary-"))
    cleanupRoots.push(cwd)
    ...
    const started = await runtime.manager.start({
      ...
      definition: {
        key: "abort-boundary",
        name: "abort boundary",
        nodes: [{ id: "active", prompt: "active", subagent_type: "explore", model: "rubato-mock/mock-1" }],
      },
    })
    await within(runner.whenStarted(1))
    ...
    try {
      ...
      const survivor = await runtime.manager.start({
        ...
        definition: {
          ...
          nodes: [{ id: "survivor", prompt: "survive", subagent_type: "explore", model: "rubato-mock/mock-1" }],
        },
      })
  ...
  test("#given an active run #when detach switches sessions before its task settles #then the old scheduler terminates without resolving ownership against the new session", async () => {
    // given
    const cwd = fs.mkdtempSync(join(tmpdir(), "rubato-runtime-dag-detach-"))
    cleanupRoots.push(cwd)
    const runner = new ScriptedRunner()
    const pi = new FakeExtensionAPI()
    const engine = composeTaskEngine({
  ...
  test("#given a throwing durable-event subscriber #when a run publishes checkpoints #then the scheduler and other subscribers still reach completion", async () => {
    // given
    const cwd = fs.mkdtempSync(join(tmpdir(), "rubato-runtime-dag-subscriber-"))
    cleanupRoots.push(cwd)
    const runner = new ScriptedRunner()
    const pi = Object.assign(new FakeExtensionAPI(), {
      rpc: {
        emit: (name: string) => {
          if (name === "rubato.dag.event") throw new Error("subscriber exploded")
        },
        handle: () => undefined,
      },
    })
  ...
  test("#given a live adapter DAG #when shutdown pauses it and a later adapter starts #then the paused event is durable and the run is claimed, resumed, and completed", async () => {
    // given
    const cwd = fs.mkdtempSync(join(tmpdir(), "rubato-runtime-dag-recovery-"))
    cleanupRoots.push(cwd)
  ...
  test("#given a paused run and a non-default subscriber ring #when the assembled runtime resumes it #then shipped RPC receives the recovery scheduler overflow", async () => {
    // given
    const cwd = fs.mkdtempSync(join(tmpdir(), "rubato-runtime-dag-recovery-ring-"))
    cleanupRoots.push(cwd)
    ...
    const baseConfig = loadRubatoConfig({ cwd }).config
    const engine = composeTaskEngine({
      pi,
      rubatoConfig: {
        ...baseConfig,
        task: RubatoTaskSettingsSchema.parse({ dag: { subscriber_ring: 1 } }),
      },
      cwd,
      sharedParentTools: () => [],
      runnerFactories: { inProcess: () => runner, process: () => runner },
  ...
  test("#given a paused adapter DAG whose lease holder PID is still live #when another adapter starts #then it never claims or resumes the run", async () => {
    // given
    const cwd = fs.mkdtempSync(join(tmpdir(), "rubato-runtime-dag-live-lease-"))
    cleanupRoots.push(cwd)
...
describe("dag runtime node spawn policy", () => {
  test("#given a runtime with a denying node spawn policy #when an agent-routed node is admitted #then the node fails with the denial and no child starts", async () => {
    // given
    const cwd = fs.mkdtempSync(join(tmpdir(), "rubato-runtime-dag-runtime-policy-"))
    cleanupRoots.push(cwd)
    const runner = new ScriptedRunner()
    const runnerFactories: TaskRunnerFactories = {
      inProcess: () => runner,
    ...
    const started = await runDagTool(
      ...
      },
      {
        ...
        definition: {
          key: "policy-denied",
          name: "policy denied",
          nodes: [{ id: "review", prompt: "review the plan", subagent_type: "momus", model: "rubato-mock/mock-1" }],
        },
      },
    )
    if (started.details.kind !== "started") throw new Error("expected dag start")
...
describe("assembled DAG runtime control verbs", () => {
  async function controlFixture(name: string, nodes: readonly { readonly id: string; readonly dependsOn?: readonly string[] }[]) {
    const cwd = fs.mkdtempSync(join(tmpdir(), `rubato-runtime-dag-${name}-`))
    cleanupRoots.push(cwd)
    const runner = new ScriptedRunner()
    ...
    const pi = Object.assign(new FakeExtensionAPI(), {
      rpc: {
        emit: (channel: string, data: unknown) => {
          if (channel !== "rubato.dag.event") return
  ...
  test("#given a settled run #when the runtime amend entry point edits a node #then only the changed node re-runs and the run re-enters", async () => {
    ...
    await within(runtime.amend(runId, {
      ...
      name: "amend-reentry",
      nodes: [
        { id: "plan", prompt: "do plan", subagent_type: "explore", model: "rubato-mock/mock-1" },
        { id: "build", prompt: "do build CAREFULLY", subagent_type: "explore", model: "rubato-mock/mock-1", dependsOn: ["plan"] },
      ],
    }), 5_000)
    await within(runner.whenStarted(3), 5_000)

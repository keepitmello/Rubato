...
function tempProject(): string {
  const directory = fs.mkdtempSync(join(tmpdir(), "rubato-dag-rpc-"))
...
describe("dag rpc handlers", () => {
  describe("#given a session that owns dag runs", () => {
    it("#when rubato.dag.list is called #then it returns session-scoped summaries newest first", async () => {
      // given
      let clock = 1_000
      ...
      const value = expectOk<{ readonly runs: readonly { readonly runId: string }[] }>(
        await handlers.call("rubato.dag.list", {}),
      )
    ...
    it("#when rubato.dag.list requests a limit above the maximum #then the limit clamps to 256", async () => {
      // given
      const target = engine()
      ...
      // when
      await spiedHandlers.call("rubato.dag.list", { limit: 5_000 })
      const value = expectOk<{ readonly runs: readonly unknown[]; readonly limit: number }>(
        await handlers.call("rubato.dag.list", { limit: 5_000 }),
      )
    ...
    it("#when rubato.dag.list filters by status #then only matching runs come back", async () => {
      // given
      const target = engine()
      await startRun(target, { key: "alpha", name: "alpha run" })
      const handlers = wire(target)
      ...
      const pending = expectOk<{ readonly runs: readonly unknown[] }>(
        await handlers.call("rubato.dag.list", { statuses: ["pending"] }),
      )
      const completed = expectOk<{ readonly runs: readonly unknown[] }>(
        await handlers.call("rubato.dag.list", { statuses: ["completed"] }),
      )
    ...
    it("#when rubato.dag.snapshot is called #then the run snapshot round-trips", async () => {
      // given
      const target = engine()
      ...
      const value = expectOk<{ readonly runId: string; readonly name: string; readonly nodes: readonly unknown[] }>(
        await handlers.call("rubato.dag.snapshot", { runId }),
      )
    ...
    it("#when rubato.dag.history pages a run #then it walks the ledger with an exclusive sinceSeq", async () => {
      // given
      const target = engine()
      const runId = await startRun(target, { key: "alpha", name: "alpha run" })
      ...
      // when
      const firstPage = expectOk<DagRpcHistoryPage>(await handlers.call("rubato.dag.history", { runId, limit: 3 }))
      const secondPage = expectOk<DagRpcHistoryPage>(
        await handlers.call("rubato.dag.history", { runId, sinceSeq: firstPage.nextSinceSeq, limit: 3 }),
      )
    ...
    it("#when rubato.dag.history requests a limit above the maximum #then the limit clamps to 1000", async () => {
      // given
      const target = engine()
      const runId = await startRun(target, { key: "alpha", name: "alpha run" })
      const handlers = wire(target)
      ...
      // when
      await spiedHandlers.call("rubato.dag.history", { runId, limit: 9_999 })
    ...
    it("#when events are appended while a bounded page walk is in flight #then the page set does not shift", async () => {
      ...
      const handshake = expectOk<DagRpcSubscribeHandshake>(
        await handlers.call("rubato.dag.subscribe", { runId, limit: 2 }),
      )
      ...
      while (hasMore) {
        ...
        const page = expectOk<DagRpcHistoryPage>(
          await handlers.call("rubato.dag.history", {
            runId,
            sinceSeq: cursor,
    ...
    it("#when rubato.dag.subscribe is called #then highWaterSeq is captured before the page is read", async () => {
      // given
      const target = engine()
      const runId = await startRun(target, { key: "alpha", name: "alpha run" })
      const append = appender(target, runId)
      ...
      // when
      const handshake = expectOk<DagRpcSubscribeHandshake>(await racing.call("rubato.dag.subscribe", { runId }))
      ...
      expect(handshake.eventName).toBe("rubato.dag.event")
    ...
    it("#when subscribe is followed by bounded history paging #then applied seqs are contiguous to the high water mark", async () => {
      ...
      // when
      const handshake = expectOk<DagRpcSubscribeHandshake>(
        await handlers.call("rubato.dag.subscribe", { runId, limit: 3 }),
      )
    ...
    it("#when history filters by lane and type #then the filters reach the ledger read", async () => {
      ...
      // when
      const boundary = expectOk<DagRpcHistoryPage>(await handlers.call("rubato.dag.history", { runId, lane: "boundary" }))
      const started = expectOk<DagRpcHistoryPage>(
        await handlers.call("rubato.dag.history", { runId, types: ["dag.run.started"] }),
      )
  ...
  describe("#given a request the engine must reject", () => {
    it("#when a foreign session asks for a run #then every run-scoped handler answers run_not_owned", async () => {
      ...
      // when
      const snapshot = expectError(await handlers.call("rubato.dag.snapshot", { runId }))
      const history = expectError(await handlers.call("rubato.dag.history", { runId }))
      const subscribe = expectError(await handlers.call("rubato.dag.subscribe", { runId }))
      const listed = expectOk<{ readonly runs: readonly unknown[] }>(await handlers.call("rubato.dag.list", {}))
    ...
    it("#when the run id is unknown #then the envelope carries run_not_found instead of throwing", async () => {
      ...
      // when
      const snapshot = expectError(await handlers.call("rubato.dag.snapshot", { runId: "run-missing" }))
      const history = expectError(await handlers.call("rubato.dag.history", { runId: "run-missing" }))
      const subscribe = expectError(await handlers.call("rubato.dag.subscribe", { runId: "run-missing" }))
    ...
    it("#when params are malformed #then the envelope carries invalid_arguments", async () => {
      ...
      // when
      const missingRunId = expectError(await handlers.call("rubato.dag.snapshot", {}))
      const badSince = expectError(await handlers.call("rubato.dag.history", { runId, sinceSeq: -1 }))
      const badLimit = expectError(await handlers.call("rubato.dag.list", { limit: 0 }))
      const badLane = expectError(await handlers.call("rubato.dag.history", { runId, lane: "sideways" }))
      const badStatus = expectError(await handlers.call("rubato.dag.list", { statuses: ["exploded"] }))
      const badPayload = expectError(await handlers.call("rubato.dag.subscribe", "not-an-object"))

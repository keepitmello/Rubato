...
import { RubatoTaskSettingsSchema, loadRubatoConfig } from "@rubato/config-core"
...
function tempProject(): string {
  const dir = mkdtempSync(join(tmpdir(), "rubato-runtime-task-"))
  tempRoots.push(dir)
  return dir
}
...
function wiredBridge(): {
  ...
  const engine = composeTaskEngine({ pi, rubatoConfig: loadRubatoConfig({ cwd }).config, cwd, sharedParentTools: () => [] })
...
async function seedTeamRuntime(cwd: string, leadSessionId: string, memberName = "crash"): Promise<string> {
  const loaded = loadRubatoConfig({ cwd }).config
  const settings = loaded.task ?? RubatoTaskSettingsSchema.parse({})
  const stateDir = { project_dir: cwd }
  const config = toTeamCoreConfig(settings, teamStorageBaseDir(stateDir))
  const spec = normalizeSenpiTeamSpec({
    members: [{ name: memberName, kind: "owner", model: "xai/grok-4.7", prompt: "member" }],
  }, "liveness-test")
...
describe("rubato-runtime task component wiring", () => {
  ...
  it("#given a fake ExtensionAPI boot #when the task component registers #then tools, commands, the completion renderer, and event handlers are wired", () => {
    ...
    expect(pi.handlers.map((handler) => handler.event).sort()).toEqual(
      [...TASK_EVENTS, "session_start"].sort(),
    )
  })
  ...
  it("#given the rubato-task flag is false #when the component registers #then only the unconditional hygiene sweep handler is wired", () => {
    // given
    const pi = new FakeExtensionAPI()
    const logger = createLogger()
    pi.setFlag("rubato-task", false)
    ...
    expect(logger.entries).toContainEqual({ level: "info", message: "Rubato task component disabled by flag" })
  })

  it("#given a malformed rubato.json #when the component registers #then it boots with defaults and still wires the tools", () => {
    // given a project whose .rubato/rubato.json is invalid JSON
    const project = tempProject()
    mkdirSync(join(project, ".rubato"), { recursive: true })
    writeFileSync(join(project, ".rubato", "rubato.json"), "{ not valid json ", "utf8")
    const pi = new FakeExtensionAPI()
    const logger = createLogger()
  ...
  it("#given a session start #when reconciliation runs #then reattach, cleanup, mailbox reclaim, notification retry, and lead polling stay ordered", async () => {
    ...
    const pi = new FakeExtensionAPI()
    const logger = createLogger()
    const base = composeTaskEngine({ pi, rubatoConfig: loadRubatoConfig({ cwd }).config, cwd, sharedParentTools: () => [] })
    const order: string[] = []

 ...
 * and validated directly (recorded as the rubato-json / inline source); a named spec is loaded from the
 * project `.rubato/teams` + rubato.json registry, where an unknown or failed name throws the recorded spec
...
export async function resolveTeamSpec(
  ...
  projectRoot: string,
  rubatoTeams: Record<string, unknown> | undefined,
  ...
  if (input.inlineSpec !== undefined) {
    ...
    return { spec, source: "rubato-json" }
  }
  ...
  const registry = await loadTeamRegistry({ projectRoot, ports, ...(rubatoTeams !== undefined ? { rubatoTeams } : {}) })
  ...
  throw new SenpiTeamSpecError(
    `team '${teamName}' not found in project .rubato/teams or rubato.json.${declaredSuffix}${brokenSuffix}`,
    "INVALID_SPEC",
    teamName,
  )
}

...
import { loadSenpiRubatoConfig } from "./index"

const roots: string[] = []

function fixture(): { readonly home: string; readonly project: string } {
  const root = mkdtempSync(join(tmpdir(), "rubato-runtime-config-resolution-"))
  roots.push(root)
...
function writeConfig(home: string, config: Record<string, unknown>): void {
  writeFileSync(join(home, ".rubato", "rubato.jsonc"), JSON.stringify(config), "utf8")
}
...
describe("loadSenpiRubatoConfig", () => {
  test("#given the current top-level-only Senpi categories and agents config #when resolved #then it preserves the established configuration exactly", () => {
    ...
    // when
    const result = loadSenpiRubatoConfig({ cwd: project, env: { HOME: home }, platform: "linux" })
  ...
  test("#given base and activated Senpi profile category settings #when resolved #then the Senpi profile override wins", () => {
    ...
    // when
    const result = loadSenpiRubatoConfig({ cwd: project, env: { HOME: home, RUBATO_PROFILE: "focused" }, platform: "linux" })

...
async function makeRuntimeFixture(): Promise<{
  ...
  const repoRoot = await makeTempDir("rubato-runtime-runtime-repo-")
  const sourceDist = join(repoRoot, "packages", "lsp-daemon", "dist")
  const targetDist = join(repoRoot, "packages", "rubato-runtime", "plugin", "runtime", "lsp-daemon", "dist")
  ...
  await writeText(join(sourceDist, "package.json"), JSON.stringify({ name: "@rubato/lsp-daemon", version: "0.1.0", type: "module" }))
...
afterEach(async () => {
  delete process.env.RUBATO_RUNTIME_STAGE_FAIL_AFTER_BACKUP
...
describe("Senpi LSP daemon runtime staging", () => {
  ...
  test("#given an existing staged runtime #when replacement fails after backup #then prior bytes are restored", async () => {
    ...
    process.env.RUBATO_RUNTIME_STAGE_FAIL_AFTER_BACKUP = "1"

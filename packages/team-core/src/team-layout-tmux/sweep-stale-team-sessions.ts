...
export const TEAM_SESSION_PATTERN = new RegExp(`^rubato-team-(${UUID_V4ISH_PATTERN})$`)
...
async function listTeamSessionsViaTmux(tmuxPath: string): Promise<string[]> {
	const { runTmuxCommand } = await import("@rubato/tmux-core")
...
export async function sweepStaleTeamSessionsWith(
	...
	const { sweepTmuxSessionsWith } = await import("@rubato/tmux-core")

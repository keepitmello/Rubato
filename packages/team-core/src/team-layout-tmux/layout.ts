import { isServerRunning, runTmuxCommand, type TmuxCommandResult } from "@rubato/tmux-core"
...
const TEAM_PANE_TITLE_PREFIX = "rubato-team-"
const RUBATO_ATTACH_SERVER_URL_OPTION = "@rubato_attach_server_url"
const RUBATO_ATTACH_SESSION_ID_OPTION = "@rubato_attach_session_id"
...
async function createTeamLayoutInCallerWindow(
  ...
  for (const member of members) {
    ...
    await deps.runTmuxCommand(tmuxPath, ["set-option", "-p", "-t", paneId, RUBATO_ATTACH_SERVER_URL_OPTION, serverUrl])
    await deps.runTmuxCommand(tmuxPath, ["set-option", "-p", "-t", paneId, RUBATO_ATTACH_SESSION_ID_OPTION, member.sessionId])
...
export async function removeTeamLayout(
  ...
  try {
    ...
    if (cleanupTarget?.ownedSession !== false) {
      await resolvedDeps.runTmuxCommand(tmuxPath, ["kill-session", "-t", cleanupTarget?.targetSessionId ?? `rubato-team-${teamRunId}`])
      return
    }

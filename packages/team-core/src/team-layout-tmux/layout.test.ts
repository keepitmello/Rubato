...
describe("team-layout-tmux", () => {
  ...
  test("#given ownedSession=true, targetSessionId='rubato-team-xyz' #when removeTeamLayout runs #then kill-session is called with -t rubato-team-xyz (legacy behavior preserved)", async () => {
    // given
    const { removeTeamLayout } = await loadLayoutModule()
    ...
    await removeTeamLayout("run-cleanup", {
      ownedSession: true,
      targetSessionId: "rubato-team-xyz",
      focusWindowId: "@10",
      gridWindowId: "@11",
    }, tmuxMgr as never)
    ...
    // then
    const commands = getCommands()
    expect(commands).toContainEqual(["kill-session", "-t", "rubato-team-xyz"])
  })

...
const MEMBER_MAP_FILE = "task-members.json"
const LEGACY_MEMBER_MAP_FILE = "senpi-task-members.json"

export function memberTaskMapPath(runtimeDir: string): string {
  return join(runtimeDir, MEMBER_MAP_FILE)
}

function legacyMemberTaskMapPath(runtimeDir: string): string {
  return join(runtimeDir, LEGACY_MEMBER_MAP_FILE)
}
 ...
 * Existing installs may still have the pre-rename filename.
 */
export async function readMemberTaskMap(runtimeDir: string): Promise<MemberTaskMap> {
  for (const path of [memberTaskMapPath(runtimeDir), legacyMemberTaskMapPath(runtimeDir)]) {
    try {

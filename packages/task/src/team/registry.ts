...
export type TeamSpecSource = "project" | "rubato-json"
...
export type LoadTeamRegistryInput = {
  ...
  readonly rubatoTeams?: Record<string, unknown>
...
async function readProjectTeamSpecs(projectRoot: string): Promise<readonly RawProjectSpec[]> {
  const teamsDir = join(projectRoot, ".rubato", "teams")
  let entries: Dirent[]
  try {
...
/**
 * Loads named team specs from the two project-scoped sources: the raw `rubato.json` `teams` section and
 * the rubato-compatible `<projectRoot>/.rubato/teams/<name>/config.json` directory specs. The project
 * directory beats `rubato.json` on a name collision. Each candidate is normalized + parsed via
...
export async function loadTeamRegistry(input: LoadTeamRegistryInput): Promise<LoadTeamRegistryResult> {
  ...
  const rubatoTeams = input.rubatoTeams ?? {}
  for (const name of Object.keys(rubatoTeams)) {
    if (projectNames.has(name)) continue
    ingest(name, "rubato-json", Object.hasOwn(rubatoTeams, name) ? rubatoTeams[name] : undefined, input.ports, teams, errors)
  }

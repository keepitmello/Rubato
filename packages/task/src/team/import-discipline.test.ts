...
const ALLOWED_TEAM_CORE_SUBPATHS: ReadonlySet<string> = new Set([
  "@rubato/team-core/team-registry",
  "@rubato/team-core/team-mailbox",
  "@rubato/team-core/team-tasklist",
  "@rubato/team-core/team-state-store",
  "@rubato/team-core/types",
  "@rubato/team-core/config",
])

const FORBIDDEN_SPECIFIERS: readonly string[] = [
  "@rubato/team-core/team-worktree",
  "@rubato/team-core/team-layout-tmux",
  "@rubato/team-core/team-state-store/session-liveness",
  "@rubato/team-core/team-mailbox/pending-delivery-recovery",
  "@rubato/tmux-core",
]
...
describe("team layer import discipline", () => {
  ...
  test("#given team-core imports #when scanned #then only exported subpaths are used", () => {
    ...
    for (const file of files) {
      const source = readFileSync(file, "utf8")
      for (const specifier of extractImportSpecifiers(source)) {
        if (specifier.startsWith("@rubato/team-core") && !ALLOWED_TEAM_CORE_SUBPATHS.has(specifier)) {

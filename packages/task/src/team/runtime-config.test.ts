...
describe("toTeamCoreConfig", () => {
  test("#given rubato task.team bounds #when converted #then the team-core config carries the bounds and base dir", () => {
    // given
...
describe("toTeamCoreSpecSource", () => {
  ...
  test("#given a rubato-json source #when mapped #then it maps to the user slot", () => {
    // when / then
    expect(toTeamCoreSpecSource("rubato-json")).toBe("user")
  })
})

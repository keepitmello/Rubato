...
export function consoleAttachment(pid: number): ConsoleAttachment {
  const source = [
    ...
    "public static class RubatoConsoleProbe {",
    ...
    "}",
    "'@",
    "[RubatoConsoleProbe]::FreeConsole() | Out-Null",
    `$attached = [RubatoConsoleProbe]::AttachConsole(${pid})`,
    "$errorCode = [Runtime.InteropServices.Marshal]::GetLastWin32Error()",
    "$windowHandle = [RubatoConsoleProbe]::GetConsoleWindow()",
    "$windowVisible = [RubatoConsoleProbe]::IsWindowVisible($windowHandle)",
    ...
    "if ($attached) { [RubatoConsoleProbe]::FreeConsole() | Out-Null }",
  ].join("\n")
  const result = spawnSync("powershell.exe", ["-NoProfile", "-NonInteractive", "-Command", source], {
    encoding: "utf8",
    windowsHide: true,

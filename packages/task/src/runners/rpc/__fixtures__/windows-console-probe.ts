...
async function runCase(mode: ProbeMode, root: string): Promise<ProbeCase> {
  ...
  const parent = spawn(
    ...
    ["-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File", CONSOLE_HOST_PATH],
    {
      env: {
        ...process.env,
        RUBATO_PROBE_BUN: process.execPath,
        RUBATO_PROBE_SCRIPT: SCRIPT_PATH,
        RUBATO_PROBE_MODE: mode,
        RUBATO_PROBE_ROOT: root,
        RUBATO_PROBE_READY_FILE: readyPath,
        RUBATO_PROBE_STOP_FILE: stopPath,
        RUBATO_PROBE_ERROR_FILE: errorPath,
...
async function runParent(mode: ProbeMode, root: string): Promise<void> {
  const readyPath = process.env.RUBATO_PROBE_READY_FILE
  const stopPath = process.env.RUBATO_PROBE_STOP_FILE
...
async function runProbe(): Promise<void> {
  ...
  const root = realpathSync.native(mkdtempSync(join(tmpdir(), "rubato-rpc-window-probe-")))

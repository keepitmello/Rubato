...
// The task/team tool family is excluded so a child cannot spawn its own children through the same
// surface; memberScopedTools (merged afterwards) are the ONLY sanctioned bypass.
...
export function isTaskOrTeamFamilyTool(name: string): boolean {
  return name === "task" || name.startsWith("Agent") || name.startsWith("task_") || name.startsWith("team_")
}

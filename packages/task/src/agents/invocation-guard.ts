...
// session-scoped skill-invocation state is supplied by the host adapter (rubato-runtime) via
...
// .rubato/plans/*.md file (at ANY root - worktrees and external checkouts included) was touched in
...
export function evaluateInvocationGuard(agentName: string, state: SkillInvocationState): InvocationGuardVerdict {
  ...
  if (condition.requiresPlanArtifact && !state.hasPlanArtifact()) {
    return {
      kind: "deny",
      message:
        `Agent "${agentName}" is plan-gated: no plan artifact (.rubato/plans/*.md) was touched in this session, so there is no ` +

...
describe("loadAgents", () => {
  ...
  test("#given file programmatic and rubato config definitions #when loading #then rubato agents overlay wins last", () => {
    // given
    const fixture = makeFixture()
    ...
    writeText(join(fixture.project, ".rubato", "rubato.json"), `{"agents":{"finder":{"model":"rubato-model"}}}`)

    // when
    const result = loadAgents({ homeDir: fixture.home, projectDir: fixture.project })

    // then
    expect(result.agents.finder?.model).toBe("rubato-model")
  ...
  test("#given rubato json path is a directory #when loading #then read diagnostic is returned and valid agents still load", () => {
    // given
    const fixture = makeFixture()
    const configPath = join(fixture.project, ".rubato", "rubato.json")
    writeText(join(fixture.project, ".senpi", "agent", "valid.md"), agentMarkdown("valid-model", "Valid"))
...
`,
  ...
  test("#given activated Senpi agent overlays and model catalog aliases #when loading #then the Senpi profile view and concrete model chain win", () => {
    ...
    const fixture = makeFixture()
    writeText(
      join(fixture.project, ".rubato", "rubato.json"),
      JSON.stringify({
    ...
    // when
    const result = loadAgents({ homeDir: fixture.home, projectDir: fixture.project, env: { RUBATO_PROFILE: "focused" } })

    // then
    expect(result.agents.finder?.model).toBe("provider/focused")
  })

  test("#given snake case rubato agent keys #when loading #then fields normalize to camelCase definitions", () => {
    // given
    const fixture = makeFixture()
    writeText(
      join(fixture.project, ".rubato", "rubato.json"),

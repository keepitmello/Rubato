...
function fallbackObjectToString(fallback: RubatoFallbackModelObject): string {
  return fallback.variant ? `${fallback.model} ${fallback.variant}` : fallback.model
}

function flattenFallbackModels(fallbackModels: RubatoFallbackModels | undefined): readonly string[] | undefined {
...
function categoryModelCandidates(config: RubatoCategoryConfig): readonly ModelChainCandidate[] {
...
function availableCategoryNames(config: RubatoConfig): readonly string[] {
...
function promptAppendForCategory(userPromptAppend: string | undefined): string | undefined {
  return userPromptAppend === undefined || userPromptAppend.length === 0 ? undefined : userPromptAppend
}
...
export function resolveCategory<TModel extends SenpiModelPort>(
  ...
  const availableCategories = availableCategoryNames(rubatoConfig)
  const userConfig = rubatoConfig.categories ? getOwnRecordValue(rubatoConfig.categories, categoryName) : undefined
  if (userConfig?.disable === true) {
    return {
      kind: "disabled",
      category: categoryName,
      reason: `Category "${categoryName}" is disabled by rubato.json`,
      availableCategories: gatedAvailableCategories(rubatoConfig, senpiModelRegistry),
    }
  }

  if (!userConfig) {
    return { kind: "not_found", category: categoryName, availableCategories: gatedAvailableCategories(rubatoConfig, senpiModelRegistry) }
  }
  ...
  const gatedCategories = availableCategoryNames(rubatoConfig)
  // Canonical models[] wins over the legacy model + fallback_models branch when present.
  ...
  const prompt_append = promptAppendForCategory(userConfig?.prompt_append)
  ...
  const chainCandidates: readonly ModelChainCandidate[] = []

...
if (evidenceDir === undefined) throw new Error("Usage: bun packages/task/scripts/manual-agents-qa.ts <evidence-dir>")
...
const userConfigPath = join(homeDir, ".rubato", "rubato.jsonc")
...
const projectConfigPath = join(projectDir, ".rubato", "rubato.json")
writeText(
  projectConfigPath,
  JSON.stringify({
    agents: {
      finder: {
        model: "rubato-override",
...
if (finder.model !== "rubato-override") throw new Error("rubato.json model override did not win")

 ...
 * provisioned and ran the Rubato-pinned ast-grep 0.43.0 binary at
 * ~/.rubato/runtime/ast-grep/<platform-arch>/sg. The matrix below pins all three probed
...
const PINNED_SG_PATH = join(astGrepRuntimeDir(join(homedir(), ".rubato")), process.platform === "win32" ? "sg.exe" : "sg");
...
function fixtureRepo(contents = 'console.log("hello");\nconst x = 1;\nconsole.log(x);\n'): string {
  const dir = mkdtempSync(join(tmpdir(), "rubato-scan-"));

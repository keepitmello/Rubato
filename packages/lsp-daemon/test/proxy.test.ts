...
describe("mcp stdio proxy", () => {
	...
	it("#given OpenCode LSP env #when tools are proxied #then daemon requests use the env-derived typed context", async () => {
		...
		const rubatoConfig = join(projectRoot, ".rubato", "lsp.json");
		const omoClientConfig = join(projectRoot, ".rubato", "lsp-client.json");
		...
		process.env["LSP_TOOLS_MCP_PROJECT_CONFIG"] = [opencodeConfig, rubatoConfig, omoClientConfig].join(delimiter);

...
function fallbackTempPaths(): DaemonPaths {
	const root = mkdtempSync("/tmp/rubato-lsp-security-smoke-");
	tempDirectories.push(root);
	const longBase = join(root, "state", "..", "state", "x".repeat(130));
	const endpointTmp = join(root, "tmp");
	return daemonPaths(
		{ [RUBATO_LSP_DAEMON_DIR]: longBase },
		...
		socket.once("connect", () => {
			socket.write(
				encodeJsonLine({
					...
					id: 1,
					method: "tools/call",
					params: { _rubato: { protocolVersion: 1, token }, name: "status", arguments: { _context: existingContext(paths.dir) } },
				}),
			);
		});
			...
			socket.once("connect", () => {
				socket.write(
					encodeJsonLine({
						...
						id: 9,
						method: "tools/call",
						params: { _rubato: { protocolVersion: 2, token }, name: "status", arguments: {} },
					}),
				);
			});
			socket.on("data", (chunk) => decoder.push(chunk));

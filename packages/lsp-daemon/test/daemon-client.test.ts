...
describe("currentRequestContext", () => {
	it("#given unrelated env #when building request context #then constructs exact typed Codex defaults without forwarded env", () => {
		const context = currentRequestContext({
			LSP_TOOLS_MCP_PROJECT_CONFIG: ".opencode/lsp.json:.rubato/lsp.json",
			LSP_TOOLS_MCP_USER_CONFIG: "~/.rubato/lsp.json",
			LSP_TOOLS_MCP_INSTALL_DECISIONS: "~/.rubato/lsp-install-decisions.json",
			PATH: "/usr/bin",
			HOME: "/home/me",
		});

...
function extractToken(message: unknown): string | null {
	...
	const rubato = Reflect.get(params, "_rubato");
	if (!rubato || typeof rubato !== "object" || Array.isArray(rubato)) return null;
	const expected = authEnvelope("probe");
	const token = Reflect.get(rubato, "token");
	return Reflect.get(rubato, "protocolVersion") === expected.protocolVersion && typeof token === "string" ? token : null;
}

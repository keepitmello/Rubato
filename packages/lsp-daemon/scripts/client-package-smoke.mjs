...
const cliResolved = await import.meta.resolve("@rubato/lsp-daemon/cli");
...
const runtime = resolveDaemonRuntime({
	[RUBATO_LSP_DAEMON_CLI]: cliPath,
	[RUBATO_LSP_DAEMON_VERSION]: version,
}, { cliPath, version });
process.env[RUBATO_LSP_DAEMON_DIR] = omoRoot;
process.env[RUBATO_LSP_DAEMON_CLI] = cliPath;
process.env[RUBATO_LSP_DAEMON_VERSION] = version;
...
const clientModule = await import("@rubato/lsp-daemon/client");
...
const rootImport = await rejected("@rubato/lsp-daemon");
const unknownImport = await rejected("@rubato/lsp-daemon/unknown");
const deepImport = await rejected("@rubato/lsp-daemon/dist/cli.js");
const serverSymbols = [
	"runMcpStdioProxy",
	"startDaemonServer",
	"ensureDaemonRunning",
...
writeFileSync(output, JSON.stringify({
	result: "PASS",
	envConstants: [RUBATO_LSP_DAEMON_CLI, RUBATO_LSP_DAEMON_DIR, RUBATO_LSP_DAEMON_VERSION].sort(),
	runtime,
...
try {
	...
	const omoRoot = join(workRoot, "rubato-lsp-daemon");
	...
	finalResult = {
		...
		scans: {
			clientJsNoWorkspaceDeps: !clientJs.includes("@rubato/"),
			clientDtsNoWorkspaceDeps: !clientDts.includes("@rubato/"),
			noRepositoryPathCoupling: !clientJs.includes(repoRoot) && !clientDts.includes(repoRoot),
			clientDtsHasNoPackageRootImport: !clientDts.includes("@rubato/lsp-daemon"),
		},
	...
	if (process.env["RUBATO_KEEP_CLIENT_PACKAGE_SMOKE"] !== "1") rmSync(workRoot, { recursive: true, force: true });
}

...
const { daemonPaths: resolveDaemonPaths, RUBATO_LSP_DAEMON_DIR } = await import(
...
try {
	const paths = resolveDaemonPaths(
		{ [RUBATO_LSP_DAEMON_DIR]: join(tempRoot, "daemon") },
...
function redactAuth(message) {
	...
	if (cloned?.params?._rubato?.token) cloned.params._rubato.token = "redacted";
	return cloned;
}

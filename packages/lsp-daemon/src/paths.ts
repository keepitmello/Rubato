...
export class InvalidDaemonDirectoryError extends Error {
	...
	constructor(directory: string) {
		super(`${RUBATO_LSP_DAEMON_DIR} must be an absolute path`);
...
export function daemonBaseDir(
	...
	const override = env[RUBATO_LSP_DAEMON_DIR];
	...
	return platform.path.resolve(platform.path.join(platform.homedir(), ".rubato", "lsp-daemon"));
}
...
function resolveSocketPath(dir: string, version: string, platform: DaemonPlatform): string {
	...
	if (platform.platform === "win32") {
		...
		return `\\\\.\\pipe\\rubato-lsp-${version}-${digest}`;
	}

	const natural = platform.path.join(canonicalVersionDir, "daemon.sock");
	if (natural.length < MAX_SOCKET_PATH_LENGTH) return natural;
	return platform.path.join(platform.tmpdir(), `rubato-lsp-${version}-${shortDigest(canonicalVersionDir)}`, "daemon.sock");
}

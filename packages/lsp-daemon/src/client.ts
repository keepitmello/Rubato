...
export const RUBATO_LSP_DAEMON_DIR = "RUBATO_LSP_DAEMON_DIR";
export const RUBATO_LSP_DAEMON_CLI = "RUBATO_LSP_DAEMON_CLI";
export const RUBATO_LSP_DAEMON_VERSION = "RUBATO_LSP_DAEMON_VERSION";
...
export function resolveDaemonRuntime(
	...
	const cliOverride = env[RUBATO_LSP_DAEMON_CLI];
	const versionOverride = env[RUBATO_LSP_DAEMON_VERSION];
	...
	if (hasCliOverride !== hasVersionOverride) {
		throw new InvalidRuntimeOverrideError(
			"paired_values_required",
			`${RUBATO_LSP_DAEMON_CLI} and ${RUBATO_LSP_DAEMON_VERSION} must be set together`,
		);
	}
	...
	if (!isAbsolute(cliOverride)) {
		throw new InvalidRuntimeOverrideError(
			"cli_must_be_absolute",
			`${RUBATO_LSP_DAEMON_CLI} must be an absolute path to an existing regular file`,
		);
	}

	const stat = statSync(cliOverride, { throwIfNoEntry: false });
	if (!stat) {
		throw new InvalidRuntimeOverrideError("cli_not_found", `${RUBATO_LSP_DAEMON_CLI} points to a missing file`);
	}
	if (!stat.isFile()) {
		throw new InvalidRuntimeOverrideError("cli_not_file", `${RUBATO_LSP_DAEMON_CLI} must point to a regular file`);
	}

...
import { type DaemonPaths, daemonPaths, RUBATO_LSP_DAEMON_DIR } from "./paths.js";
...
export async function reapStaleDaemonVersions(
	...
	for (const entry of [...entries].sort((left, right) => left.name.localeCompare(right.name))) {
		...
		const siblingPaths = daemonPaths({ [RUBATO_LSP_DAEMON_DIR]: baseDir }, { cliPath: ownPaths.cliPath, version });
		results.push(

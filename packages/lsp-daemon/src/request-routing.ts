import { handleLspMcpRequest } from "@rubato/lsp-core/mcp";
import { parseLspRequestContext, type RequestContext, runWithRequestContext } from "@rubato/lsp-core/request-context";
import { isPlainRecord } from "@rubato/mcp-stdio-core/record";

import { authenticateMessage, RUBATO_DAEMON_PROTOCOL_VERSION } from "./ipc-protocol.js";
...
export function handleDaemonMessage(raw: unknown, state: DaemonRouteState): Promise<unknown> {
	...
	if (authenticated.method === "rubato/ping") {
		return Promise.resolve({
			jsonrpc: "2.0",
			id: authenticated.id,
			result: { protocolVersion: RUBATO_DAEMON_PROTOCOL_VERSION, ...state.owner },
		});
	}

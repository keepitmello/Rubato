...
export function resolveDaemonRuntime(env: NodeJS.ProcessEnv, defaults: DaemonRuntimeDefaults): DaemonRuntime {
	...
	try {
		...
		throw new InvalidRuntimeOverrideError(
			"cli_not_found",
			`${RUBATO_LSP_DAEMON_CLI} must name an existing regular file`,
		);
	}
	if (!cliStats.isFile()) {
		throw new InvalidRuntimeOverrideError("cli_not_file", `${RUBATO_LSP_DAEMON_CLI} must name an existing regular file`);
	}

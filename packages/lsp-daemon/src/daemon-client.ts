...
import { type LspRequestContext, parseLspRequestContext } from "@rubato/lsp-core/request-context";
import type { ToolExecutionResult } from "@rubato/lsp-core/tools";
	...
	return new Promise((resolve, reject) => {
		...
		const cancelPayload = (): string =>
			encodeJsonLine({
				...
				params: { _rubato: authEnvelope(token), id: requestId },
			});
		const finish = (run: () => void): void => {
			if (settled) return;
			settled = true;
		...
		socket.once("connect", () => {
			const payload = encodeJsonLine({
				...
				id: requestId,
				method: "tools/call",
				params: { _rubato: authEnvelope(token), name, arguments: args },
			});

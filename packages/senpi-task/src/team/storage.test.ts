...
describe("team storage layout", () => {
  test("#given a project state dir config #when the base dir is resolved #then it lands under the senpi-task state dir", () => {
    ...
    // then
    expect(baseDir).toBe(join(projectDir, ".rubato", "senpi-task", "teams"))
  })
  ...
  test("#given a team run id #when runtime dirs are resolved #then runtime and tasks dirs live under the base dir", () => {
    ...
    // then
    expect(dirs.baseDir).toBe(join(projectDir, ".rubato", "senpi-task", "teams"))
  ...
  test("#given the discovery-only project spec path #when resolved #then it maps to the rubato-compatible .rubato/teams path", () => {
    // given
    const projectDir = makeProjectDir()
    ...
    // then
    expect(specPath).toBe(join(projectDir, ".rubato", "teams", "research-team", "config.json"))

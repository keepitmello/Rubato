...
describe("normalizeSenpiTeamSpec", () => {
  ...
  test("#given a name-less rubato.json team value #when normalized #then it takes the record key as its name", () => {
    // given
    const rawSpec = { members: [{ kind: "subagent_type", subagent_type: "ultraworker" }] }
  ...
  test("#given a member literally named 'lead' #when normalized #then it is rejected with a typed diagnostic", () => {
    ...
    const rawSpec = {
      members: [
        { kind: "subagent_type", subagent_type: "ultraworker", name: "lead" },
  ...
  test("#given the callerTeamLead option #when normalized #then it is rejected before any member is spawned", () => {
    ...
    let caught: unknown
    try {
      normalizeSenpiTeamSpec(rawSpec, "with-caller-lead", { callerTeamLead: { agentTypeId: "ultraworker" } })
    } catch (error) {
      caught = error
    }

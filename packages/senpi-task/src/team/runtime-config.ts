...
/**
 * Projects the Rubato `task.team` bounds onto a team-core config, pinning `base_dir` to the senpi team
 * storage root so team-core writes runtime state under the senpi state dir instead of `~/.rubato`.
 */
export function toTeamCoreConfig(task: RubatoTaskSettings, baseDir: string): TeamCoreConfig {
  return {
...
// spec files from `rubato.json` inline specs. The inline specs land in the non-project ("user") slot.

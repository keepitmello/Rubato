...
// the child receives EXACTLY one canonical sentence carrying one .rubato/plans/*.md path - no claims,
...
const PLAN_PATH_GLOBAL = /[^\s"'`()\[\]]*\.rubato[\\/]plans[\\/][^\s"'`()\[\]/\\]+\.md/gi

export const PLAN_REVIEW_DENY_MESSAGE =
  "momus requires exactly one .rubato/plans/*.md path: include the plan path in the prompt, or touch the plan file in this session first."
...
// normalized `.rubato/plans/<name>.md` suffix, which keeps worktree-rooted recordings matchable.

...
function resolvedAgentMode(
  ...
  return toExecutionMode(deps.agents[target.subagentType]?.executionMode) ?? deps.rubatoConfig.agents?.[target.subagentType]?.execution_mode
}
...
function resolvedTaskExecutionMode(
  ...
  return resolveExecutionMode({
    ...
    configMode: deps.rubatoConfig.task?.default_execution_mode,
  })
}

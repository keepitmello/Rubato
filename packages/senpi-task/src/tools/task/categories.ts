import type { RubatoCategoryConfig, RubatoConfig } from "@rubato/config-core"
...
// Dynamic category surface for the tool description: builtin categories merged with rubato.json ones,
// disabled categories dropped, user descriptions winning over the builtin default text.
export function listTaskCategories(config: RubatoConfig): readonly TaskCategoryInfo[] {
  const userCategories: Readonly<Record<string, RubatoCategoryConfig>> = config.categories ?? {}

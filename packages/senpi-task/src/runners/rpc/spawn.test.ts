...
describe("buildChildArgs", () => {
  test("#given a spec with model and extensions #when building child args #then no-extensions leads, each -e follows, then --model", () => {
    // when
    const args = buildChildArgs({ ...baseSpec, model: "rubato-mock/mock-1", extensions: ["/tmp/a.ts", "/tmp/b.ts"] })
    // then
    expect(args).toEqual(["--no-extensions", "--extension", "/tmp/a.ts", "--extension", "/tmp/b.ts", "--model", "rubato-mock/mock-1"])
  })
  ...
  test("#given a spec with a valid variant #when building child args #then --thinking follows --model", () => {
    // when
    const args = buildChildArgs({ ...baseSpec, model: "rubato-mock/mock-1", variant: "xhigh" })
    // then
    expect(args).toEqual(["--no-extensions", "--model", "rubato-mock/mock-1", "--thinking", "xhigh"])
  })

  test("#given a spec with high reasoning effort #when building child args #then it maps to senpi high", () => {
    // when
    const args = buildChildArgs({ ...baseSpec, model: "rubato-mock/mock-1", variant: "high" })
    // then
    expect(args).toEqual(["--no-extensions", "--model", "rubato-mock/mock-1", "--thinking", "high"])
  })

  test("#given the rubato.json reasoningEffort none as variant #when building child args #then it maps to senpi off", () => {
    // when
  ...
  test("#given an unknown variant #when building child args #then no --thinking flag is emitted", () => {
    // when
    const args = buildChildArgs({ ...baseSpec, model: "rubato-mock/mock-1", variant: "ultra" })
    // then
    expect(args).toEqual(["--no-extensions", "--model", "rubato-mock/mock-1"])
  })
})
...
describe("buildRpcSpawn spawn strategy", () => {
  test("#given a Windows npm senpi installation #when building an RPC child #then Node launches the npm package CLI without shell forwarding", () => {
    ...
    try {
      ...
      const descriptor = buildRpcSpawn(
        { ...baseSpec, model: "rubato-mock/mock-1" },
        {
          isBunBinary: false,
  ...
  test("#given a resolvable senpi executable #when building #then it spawns the EXECUTABLE in rpc mode (not the loader-hijacked rpc-entry)", () => {
    // when
    const descriptor = buildRpcSpawn(
      { ...baseSpec, model: "rubato-mock/mock-1", extensions: ["/tmp/mock.ts"] },
    ...
    expect(descriptor.args).toContain("rubato-mock/mock-1")
  ...
  test("#given a bun runtime with a resolvable sibling executable #when building #then the sibling binary runs rpc mode with threaded args", () => {
    ...
    expect(descriptor.args).toEqual(["--mode", "rpc", "--no-extensions", "--model", "rubato-mock/mock-1"])
  ...
  test("#given a generic child spawned by a member #when building #then member identity and extension do not leak", () => {
    // given
    const memberExtension = "/tmp/rubato-member.js"
  ...
  test("#given member extension env w2mem #when building #then identity config and task id reach the child without overriding isolation", () => {
    ...
    // when
    const descriptor = buildRpcSpawn(
      { ...baseSpec, extensions: ["/tmp/rubato-member.js"], memberEnv },
      {
        isBunBinary: false,
        execPath: "/usr/bin/node",
        platform: "linux",

...
describe("category activation gating", () => {
  describe("#given a builtin category gated on claude-fable-5", () => {
    ...
    test("#when the gate model is absent but rubato.json configures the category #then the explicit entry bypasses the gate", () => {
      // given
    ...
    test("#when the gate model is absent and rubato.json only sets a description #then the gate is bypassed and the category stays listed", () => {
      // given

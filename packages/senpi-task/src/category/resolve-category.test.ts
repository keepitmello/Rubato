...
describe("resolveCategory", () => {
  test("#given a builtin category and rubato overlay #when resolved #then user config wins and prompt text is appended", () => {
    // given
  ...
  test("#given a disabled rubato category overlay #when resolved #then a disabled result explains the reason", () => {
    // given
  ...
  test("#given primary model is unavailable and rubato fallback exists #when resolved #then delegate-core fallback reaches the registry model", () => {
    // given
  ...
  test("#given category params in rubato overlay #when resolved #then child spec carries generation params and prompt append", () => {
    // given
...
describe("builtin category defaults", () => {
  test("#given ported builtin defaults #when inspected #then machine routing fields stay pinned without prose wording", () => {
    ...
    expect(defaults.map(({ config, name }) => [name, config.model, config.variant])).toEqual([
      ...
      ["artistry", "anthropic/claude-fable-5-1", "xhigh"],

...
describe("resolveCodegraphCommand", () => {
  it("prefers RUBATO_CODEGRAPH_BIN over bundled, provisioned, and PATH tiers", () => {
    // given
    const env = { RUBATO_CODEGRAPH_BIN: "/opt/codegraph/bin/codegraph" }

    // when
    const result = resolveCodegraphCommand({
      env,
      fileExists: (filePath: string) => filePath === env.RUBATO_CODEGRAPH_BIN,
  ...
  it("keeps an invalid RUBATO_CODEGRAPH_BIN override unavailable", () => {
    // given
    const env = { RUBATO_CODEGRAPH_BIN: "/nonexistent" }
  ...
  it("uses provisioned binaries before PATH", () => {
    // given
    const provisioned = "/home/me/.rubato/codegraph/bin/codegraph"
  ...
  it("returns the PATH tier with exists false when every detector fails", () => {
    ...
    const missing = {
      fileExists: () => false,
      homeDir: "/tmp/rubato-codegraph-resolve-missing-home",
      provisioned: () => null,
      requireResolve: () => {
        throw new Error("not bundled")
      },

...
function tempDir(name: string): string {
  return join(tmpdir(), `rubato-${name}-${crypto.randomUUID()}`)
}
...
describe("resolveSgBinarySync tier order", () => {
  ...
  it("#given no env override #when CODEX_HOME holds a runtime sg #then the Rubato runtime tier resolves it", () => {
    // given
    ...
    expect(result.found ? result.tier : null).toBe("rubato-runtime")

    rmSync(root, { force: true, recursive: true })
  })

  it("#given no CODEX_HOME #when the home Rubato runtime holds sg #then the slug directory resolves it", () => {
    // given
    const root = tempDir("tier-home")
    const runtimePath = astGrepStub(join(root, ".rubato", "runtime", "ast-grep", "darwin-arm64", "sg"))
    const recorder = recordingProbe({ [runtimePath]: "ast-grep 0.43.0" })
...
describe("resolveSgBinarySync failure", () => {
  ...
  it("#given nothing resolves on linux #when resolving #then the hints cover npm, cargo and Rubato provisioning", () => {
    // given
    ...
    expect(hintText.toLowerCase()).toContain("rubato")
  })

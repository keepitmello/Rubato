...
function rubatoRuntimeCandidates(options: {
  ...
  paths.push(join(options.homeDir, ".rubato", "runtime", "ast-grep", slug, binaryName))
  return paths.map((path) => candidate("rubato-runtime", path))
}
...
export function planSgCandidates(options: SgResolverOptions): SgCandidatePlan {
  ...
  return {
    ...
    beforePath: [
      ...
      ...rubatoRuntimeCandidates({ arch, env, homeDir, platform, runtimeDir: options.runtimeDir }),

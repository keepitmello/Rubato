...
/** Home directory with no Rubato runtime, so tier 2 cannot shadow the tier under test. */
const NO_RUNTIME_HOME = join(tmpdir(), "rubato-sg-probe-contract-no-home")
...
describe("sg probe contract: a failing probe never wins", () => {
  ...
  it("#given a Rubato runtime candidate that fails its probe #when resolving #then the runtime location grants no exemption", () => {
    // given: the Rubato-provisioned runtime dir holds a binary that cannot run

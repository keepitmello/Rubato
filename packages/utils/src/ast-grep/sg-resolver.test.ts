...
/** Home directory with no Rubato runtime, so tier-2 candidates cannot shadow the tier under test. */
const NO_RUNTIME_HOME = join(tmpdir(), "rubato-sg-resolver-no-runtime-home")

describe("findSgBinarySync", () => {
  it("prefers the RUBATO_AST_GREP_SG_PATH override", () => {
    // given

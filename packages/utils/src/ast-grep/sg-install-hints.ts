...
const RUBATO_PROVISION_HINT = "Start a Rubato session so the bundled ast-grep skill provisions the pinned runtime automatically"
...
export function sgInstallHints(platform: NodeJS.Platform = process.platform): readonly string[] {
  return [...platformHints(platform), RUBATO_PROVISION_HINT, ENV_OVERRIDE_HINT]
}

export function sgBinaryNotFoundMessage(platform: NodeJS.Platform = process.platform): string {
  return `ast-grep binary not found for ${platform}: no candidate passed the --version probe across the env override, Rubato runtime, skill bin cache, PATH, or Homebrew prefixes.`
}

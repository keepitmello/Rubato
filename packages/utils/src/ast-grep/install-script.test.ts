...
describe("runAstGrepSkillInstall", () => {
  test("#given install.sh exists #when provisioning runs #then RUBATO_AST_GREP_BIN_DIR targets the runtime slug", async () => {
    // given
    const invocations: Array<{ readonly command: string; readonly binDir: string | undefined }> = []
    const targetDir = astGrepRuntimeDir("/home/test/.rubato", "darwin", "arm64")
    ...
    expect(targetDir).toBe(join("/home/test/.rubato", "runtime", "ast-grep", "darwin-arm64"))
  })
  ...
  test("#given install script exits one #when provisioning runs #then the failure is returned without throwing", async () => {
    ...
    const result = await runAstGrepSkillInstall({
      ...
      skillDir: "/skills/ast-grep",
      spawnProcess,
      targetDir: "/home/test/.rubato/runtime/ast-grep/linux-x64",
    })

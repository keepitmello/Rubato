...
describe("CodeGraph zombie sweep", () => {
  ...
  it("#given a fresh daemon throttle stamp #when sweep runs without force #then workers are checked while the daemon lane stays throttled", async () => {
    // given
    const homeDir = mkdtempSync(join(tmpdir(), "rubato-codegraph-sweep-home-"))
    let providerCalls = 0
    try {
  ...
  it("#given a live daemon whose lockfile records its pid #when sweep runs #then the daemon appears in candidates and is spared", async () => {
    ...
    const projectDir = mkdtempSync(join(tmpdir(), "rubato-codegraph-daemon-proj-"))
    const calls: string[] = []
    const logs: string[] = []
    try {
  ...
  it("#given the daemon lock lives at an initialized ancestor of the --path root #when sweep runs #then the daemon is spared", async () => {
    // given
    const installDir = "/tmp/rubato-install"
    const ancestorDir = mkdtempSync(join(tmpdir(), "rubato-codegraph-daemon-ancestor-"))

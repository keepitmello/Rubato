...
describe("git-worktree", () => {
  ...
  test("#given notepad path #when formatting rubato plan changes #then does not report notepad updated", () => {
    const summary = formatFileChanges([
      { path: ".rubato/plans/work.md", added: 1, removed: 0, status: "modified" },
    ], ".rubato/notepads/work/notes.md")

    expect(summary).not.toContain("[NOTEPAD UPDATED]")
  })

  test("#given notepad path #when formatting rubato notepad changes #then reports notepad updated", () => {
    const summary = formatFileChanges([
      { path: ".rubato/notepads/work/notes.md", added: 1, removed: 0, status: "modified" },
    ], ".rubato/notepads/work/notes.md")

    expect(summary).toContain("[NOTEPAD UPDATED]")
    expect(summary).toContain(".rubato/notepads/work/notes.md")
  })

  test("#given notepad path #when formatting another rubato notepad change #then does not report active notepad updated", () => {
    const summary = formatFileChanges([
      { path: ".rubato/notepads/other/notes.md", added: 1, removed: 0, status: "modified" },
    ], ".rubato/notepads/work/notes.md")

...
describe("CodeGraph project exclusion policy", () => {
  it("excludes default tmp roots and Rubato state directories from CodeGraph projects", () => {
    // given
    const tmpWorkspace = process.platform === "win32" ? null : mkdtempSync(join("/tmp", "rubato-codegraph-excluded-"))

    try {
      // then
      if (tmpWorkspace !== null) {
        expect(shouldExcludeCodegraphProject(tmpWorkspace, { platform: "linux", tmpdir: "/tmp/rubato-current-temp" })).toEqual({
          excluded: true,
          matchedRoot: "/tmp",
          reason: "tmp-root",
        })
      ...
      expect(
        shouldExcludeCodegraphProject("/private/tmp/rubato-codegraph-scratch/repo", {
          platform: "linux",
          tmpdir: "/tmp/rubato-current-temp",
        }),
      ...
      expect(shouldExcludeCodegraphProject("/Users/alice/repo/.rubato/ulw-research/run/clones/repo")).toEqual({
        excluded: true,
        matchedRoot: ".rubato",
        reason: "rubato-state",
      })

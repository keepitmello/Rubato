...
describe("process sweep family matrix", () => {
  ...
  const posixTable: readonly ProcessInfo[] = [
    ...
    { command: `node ${pluginRoot}/components/codegraph/dist/serve.js`, pid: 301, ppid: 1 },
    // codegraph direct upstream
    {
      command: `node ${pluginRoot}/node_modules/@colbymchenry/codegraph/bin/codegraph.js serve --mcp`,
      pid: 302,
      ppid: 9999,
    },
    ...
    { command: `node ${pluginRoot}/components/lsp-daemon/dist/cli.js mcp`, pid: 304, ppid: 1 },
    // lsp-daemon mcp proxy, live parent
    { command: `node ${pluginRoot}/components/lsp-daemon/dist/cli.js mcp`, pid: 305, ppid: 200 },
    // lsp-daemon server shape: NEVER a proxy candidate even when orphaned
    { command: `node ${pluginRoot}/components/lsp-daemon/dist/cli.js daemon`, pid: 306, ppid: 1 },
    // dev-shape lsp-daemon mcp proxy via bun + src/cli.ts, dead parent
    { command: `bun ${pluginRoot}/components/lsp-daemon/src/cli.ts mcp`, pid: 307, ppid: 9999 },
  ...
  it("#given a posix process table spanning every family #when classifying #then each family selects exactly its own zombies", () => {
    ...
    const codegraph = selectZombieCodegraphProcesses(posixTable, {
      ownedRoots: [pluginRoot, installDir],
      platform: "linux",
    })
    const proxies = selectOrphanedLspDaemonProxies(posixTable, { ownedRoots: [pluginRoot], platform: "linux" })
  ...
  it("#given a windows process table spanning every family #when classifying #then windows shapes are selected per family", () => {
    // given
    const winRoot = "C:\\Users\\runner\\.codex\\plugins\\cache\\rubato-labs\\rubato\\4.15.1"
  ...
  it("#given a proxy-shaped command with a daemon token alongside mcp #when selecting proxies #then the daemon server shape is never matched", () => {
    // given
    const table: readonly ProcessInfo[] = [
      { command: `node ${pluginRoot}/components/lsp-daemon/dist/cli.js mcp daemon`, pid: 320, ppid: 1 },
      { command: `node ${pluginRoot}/components/lsp-daemon/dist/cli.js mcp`, pid: 321, ppid: 1 },
    ]

    // when
    const proxies = selectOrphanedLspDaemonProxies(table, { ownedRoots: [pluginRoot], platform: "linux" })
  ...
  it("#given an mcp token that belongs to an unrelated cli #when selecting proxies #then it is ignored", () => {
    // given
    const table: readonly ProcessInfo[] = [
      { command: `node ${pluginRoot}/components/other-tool/dist/cli.js mcp`, pid: 330, ppid: 1 },
      { command: `/usr/bin/python3 ${pluginRoot}/components/lsp-daemon/dist/cli.js mcp`, pid: 331, ppid: 1 },
    ]
...
describe("orphaned lsp-daemon proxy sweep", () => {
  it("#given an orphaned proxy and a live-parent proxy #when sweeping #then only the orphan is killed and the family stamp is written", async () => {
    // given
    const homeDir = mkdtempSync(join(tmpdir(), "rubato-lsp-proxy-sweep-"))
    const pluginRoot = "/tmp/rubato-plugin"
    const calls: string[] = []
    try {
      // when
      const result = await sweepOrphanedLspDaemonProxies({
        ...
        ownedRoots: [pluginRoot],
        platform: "linux",
        processProvider: () =>
          Promise.resolve([
            { command: "codex app-server", pid: 200, ppid: 1 },
            { command: `node ${pluginRoot}/components/lsp-daemon/dist/cli.js mcp`, pid: 401, ppid: 1 },
            { command: `node ${pluginRoot}/components/lsp-daemon/dist/cli.js mcp`, pid: 402, ppid: 200 },
          ]),
      })
      ...
      expect(result.stampFile).toBe(join(homeDir, ".rubato", "lsp-daemon", "lsp-proxy-sweep.stamp"))
      expect(existsSync(result.stampFile)).toBe(true)
      expect(existsSync(join(homeDir, ".rubato", "codegraph", "zombie-sweep.stamp"))).toBe(false)
    } finally {
  ...
  it("#given a fresh proxy family stamp #when sweeping without force #then the proxy family is throttled independently", async () => {
    // given
    const homeDir = mkdtempSync(join(tmpdir(), "rubato-lsp-proxy-throttle-"))
    try {
...
describe("stale lsp-daemon version sweep", () => {
  it("#given a stale attested daemon and a live current daemon #when sweeping #then only the stale daemon is killed", async () => {
    ...
    const homeDir = mkdtempSync(join(tmpdir(), "rubato-lsp-daemon-sweep-"))
    const baseDir = join(homeDir, ".rubato", "lsp-daemon")
    const calls: string[] = []
    try {
  ...
  it("#given a stale version whose owner pid fails attestation #when sweeping #then it is spared as a possible recycled pid", async () => {
    // given
    const homeDir = mkdtempSync(join(tmpdir(), "rubato-lsp-daemon-spare-"))
    const baseDir = join(homeDir, ".rubato", "lsp-daemon")
    const logs: string[] = []
    try {
  ...
  it("#given a stale version on Windows #when sweeping #then it is spared because ownership cannot be proven", async () => {
    // given
    const homeDir = mkdtempSync(join(tmpdir(), "rubato-lsp-daemon-win-"))
    const baseDir = join(homeDir, ".rubato", "lsp-daemon")
    try {
  ...
  it("#given a stale version whose owner pid is dead #when sweeping #then there is nothing to kill", async () => {
    // given
    const homeDir = mkdtempSync(join(tmpdir(), "rubato-lsp-daemon-dead-"))
    const baseDir = join(homeDir, ".rubato", "lsp-daemon")
    try {
  ...
  it("#given no current version can be determined #when sweeping #then the family skips conservatively", async () => {
    // given
    const homeDir = mkdtempSync(join(tmpdir(), "rubato-lsp-daemon-unknown-"))
    const baseDir = join(homeDir, ".rubato", "lsp-daemon")
    try {
  ...
  it("#given non-version entries in the base dir #when sweeping #then they are ignored", async () => {
    // given
    const homeDir = mkdtempSync(join(tmpdir(), "rubato-lsp-daemon-entries-"))
    const baseDir = join(homeDir, ".rubato", "lsp-daemon")
    try {
...
describe("lsp-daemon cli attestation", () => {
  it("#given a linux /proc cmdline with node cli.js daemon #when attesting #then it passes", async () => {
    // given
    const cmdline = Buffer.from("node\u0000/usr/lib/rubato/lsp-daemon/dist/cli.js\u0000daemon\u0000")
  ...
  it("#given a linux /proc cmdline for the mcp proxy shape #when attesting #then it fails", async () => {
    // given
    const cmdline = Buffer.from("node\u0000/usr/lib/rubato/lsp-daemon/dist/cli.js\u0000mcp\u0000")
  ...
  it("#given a macOS ps command line with node cli.js daemon #when attesting #then it passes", async () => {
    ...
    expect(
      await attestLspDaemonCliProcess(804, "darwin", {
        executeForStdout: () => Promise.resolve("node /usr/lib/rubato/lsp-daemon/dist/cli.js daemon\n"),
      }),
    ).toBe(true)
    expect(
      await attestLspDaemonCliProcess(805, "darwin", {
        executeForStdout: () => Promise.resolve("node /usr/lib/rubato/lsp-daemon/dist/cli.js mcp\n"),
      }),
    ).toBe(false)
...
describe("backward-compatible codegraph process-sweep module", () => {
  ...
  it("#given daemon identity changes after TERM #when escalation runs #then replacement PID is never killed", async () => {
    const homeDir = mkdtempSync(join(tmpdir(), "rubato-lsp-pid-reuse-"))
    const baseDir = join(homeDir, ".rubato", "lsp-daemon")

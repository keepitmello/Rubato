...
describe("CodeGraph worker process selection", () => {
  it("#given quoted real-world POSIX worker commands #when selecting zombies #then platform and provisioned workers are matched", () => {
    // given
    const claudeRoot = "/Users/yeongyu/.claude/rubato"
    const provisionedRoot = "/Users/yeongyu/.rubato/codegraph"
    const processes = [
      {
  ...
  it("#given an orphaned npm shim still owns a platform child #when selecting zombies #then the whole worker family is selected", () => {
    // given
    const root = "/Users/yeongyu/.claude/rubato"
    const processes = [
      {
  ...
  it("#given Windows platform and provisioned worker commands #when selecting zombies #then both shapes are matched", () => {
    // given
    const pluginRoot = "C:\\Users\\runner\\.codex\\plugins\\cache\\rubato-labs\\rubato\\4.19.2"
    const provisionedRoot = "C:\\Users\\runner\\.rubato\\codegraph"
  ...
  it("#given the default home roots #when discovering owned roots #then the Claude Rubato install is included", () => {
    // given

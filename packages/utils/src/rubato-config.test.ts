...
import { validateRubatoConfig } from "./rubato-config"

describe("validateRubatoConfig", () => {
  it("accepts codegraph settings with codex and opencode override blocks", () => {
    ...
    const config = {
      codegraph: {
        auto_provision: true,
        enabled: true,
        install_dir: "~/.rubato/codegraph",
        excluded_roots: ["/tmp/rubato-scratch"],
        telemetry: false,
    ...
    // when
    const result = validateRubatoConfig(config)

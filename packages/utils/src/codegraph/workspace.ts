...
export function prepareCodegraphWorkspace(
  ...
  try {
    ...
    if (existsSync(projectLink)) {
      ...
      return fallbackResult(dataRoot, projectLink, "existing .codegraph symlink points outside Rubato store")
    }

    if (!isSameFilesystem(resolvedWorkspace, dataRoot, options.sameFilesystem)) {
      ensureInPlaceFallback(projectLink)
      return fallbackResult(dataRoot, projectLink, "workspace and Rubato store are on different filesystems")
    }

...
export type CodegraphProjectExclusionReason = "custom-root" | "rubato-state" | "tmp-root"
...
function hasRubatoPathSegment(path: string): boolean {
  return path.split(/[\\/]+/).includes(".rubato")
}
...
export function shouldExcludeCodegraphProject(
  ...
  if (hasRubatoPathSegment(resolvedWorkspace)) {
    return { excluded: true, matchedRoot: ".rubato", reason: "rubato-state" }
  }

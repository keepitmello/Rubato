...
export function buildCodegraphInitGuidance(
  ...
  return [
    "Rubato CodeGraph initialization guidance:",
    "",
    `CodeGraph is not initialized for ${displayProjectPath}. Initialize it through Rubato's global local store instead of leaving a standalone project-local index.`,
    "",
    `- Link or create ${displayProjectLink} so it points at ${displayDataDir} under the Rubato store ${displayDataRoot}.`,
    `- Then run \`codegraph init\` from ${displayProjectPath} and retry the CodeGraph tool.`,
    "- Rubato's CodeGraph bootstrap does this automatically on session start; if bootstrap just ran, wait for it to finish and retry.",
  ].join("\n")
}

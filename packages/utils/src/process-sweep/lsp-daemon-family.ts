 ...
 *
 * BOUNDARY: packages/utils cannot import the vendored `@rubato/lsp-daemon`
 * package (and lsp-daemon cannot import Rubato plugin packages), so its owner record and
...
export const RUBATO_LSP_DAEMON_DIR_ENV = "RUBATO_LSP_DAEMON_DIR"
export const RUBATO_LSP_DAEMON_VERSION_ENV = "RUBATO_LSP_DAEMON_VERSION"
...
export function resolveLspDaemonBaseDir(options: LspDaemonBaseDirOptions = {}): string {
  ...
  const override = env[RUBATO_LSP_DAEMON_DIR_ENV]
  ...
  return join(homeDir, ".rubato", "lsp-daemon")
}

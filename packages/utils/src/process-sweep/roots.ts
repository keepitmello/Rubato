...
/**
 * Rubato-owned plugin roots trusted for process matching: the provisioned
 * codegraph install dir, the Claude Rubato install, the plugin root, and the
...
export function discoverCodegraphOwnedRoots(options: CodegraphOwnedRootsOptions = {}): string[] {
  ...
  addRoot(roots, join(homeDir, ".claude", "rubato"))
...
export const discoverRubatoOwnedRoots = discoverCodegraphOwnedRoots
...
function readCodexPluginCacheRoots(codexHome: string): string[] {
  ...
  for (const publisher of safeReadDir(cacheRoot)) {
    const pluginRoot = join(cacheRoot, publisher, "rubato")
    if (!existsSync(pluginRoot)) continue
    for (const version of safeReadDir(pluginRoot)) roots.push(join(pluginRoot, version))
  }
  return roots
}

...
//
// 목록을 따로 두지 않고 데스크톱의 연결 카탈로그를 그대로 읽는다. 두 벌이면
// 앱에서 환경을 지우거나 꺼도 업데이트는 옛 기계를 계속 찾아간다. 앱에서 꺼 둔
// 환경(disabledEnvironmentIds)은 뺀다. alias 를 내는 이유는 ssh 설정(ProxyJump
// 등)을 그대로 타야 해서다 — 데스크톱도 alias 로 붙는다.
//
// 카탈로그가 없거나 읽을 수 없으면 아무것도 내지 않는다. 업데이트를 막을 일이 아니다.
...
if (import.meta.main) {
  ...
  const catalog = process.argv[2] || path.join(t3Home, 'userdata/connection-catalog.json');
  let doc;

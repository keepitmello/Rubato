...
# `open` leaves $RUBATO_BOOT_SPLASH_DIR/open when it actually painted, which is
# rubato-pi.sh's cue to start the renderer.
set -eu
DIR="${RUBATO_BOOT_SPLASH_DIR-}"
# The renderer may still be booting after open, so steps go to the file, not
# the screen; the renderer picks the latest one up on its next tick.
animated() { [ -n "$DIR" ] && [ -f "$DIR/open" ]; }
...
case "${1-}" in
  open)
    if [ -n "$DIR" ] && [ -d "$DIR" ]; then : >"$DIR/open"; fi

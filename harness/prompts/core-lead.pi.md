...
Before choosing any agent's category (a `task` agent or a team roster alike), read Skill(model-guide). It is the source of truth for cognitive profiles and verifier families; the harness owns live-catalog resolution, provider preference, admission, and fallback.

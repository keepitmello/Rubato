...
| 패치 | 판정 | 사유 |
|---|---|---|
| `dist/modes/interactive/interactive-mode.js` | keep | pi-tui import 줄과 `this.chatContainer = new Container();` 앵커 그대로. 0.86.1은 같은 컨테이너에 크래시·`/bug`·캐시 워밍 자식을 더 넣지만 우리 컨테이너가 그대로 받는다. |

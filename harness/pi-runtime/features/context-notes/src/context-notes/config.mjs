...
const modeBySettings = new WeakMap();
...
export function contextModeForSession(sessionId, env = process.env) {
  if (typeof sessionId === "string" && modeBySessionId.has(sessionId)) return modeBySessionId.get(sessionId);
  return contextMode(env);
}

export function historyNotesEnabledForSession(sessionId, env = process.env) {
  return contextModeForSession(sessionId, env) === HISTORY_NOTES_MODE;
}

export function contextModeForSettings(settingsManager, env = process.env) {
  if (settingsManager && typeof settingsManager === "object" && modeBySettings.has(settingsManager)) {
    return modeBySettings.get(settingsManager);
  }
  return contextMode(env);
}

export function historyNotesEnabledForSettings(settingsManager, env = process.env) {
  return contextModeForSettings(settingsManager, env) === HISTORY_NOTES_MODE;
}

export function setContextMode(mode, env = process.env, options = {}) {
  const next = validateMode(mode);
  const sessionId = options.sessionId;
  const settingsManager = options.settingsManager;
  const propagateEnv = options.propagateEnv ?? (env === process.env);
  if (typeof sessionId === "string" && sessionId) modeBySessionId.set(sessionId, next);
  if (settingsManager && typeof settingsManager === "object") modeBySettings.set(settingsManager, next);
  if (propagateEnv) {

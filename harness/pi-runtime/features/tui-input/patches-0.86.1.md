...
| 패치 | 판정 | 사유 |
|---|---|---|
| `dist/modes/interactive/interactive-mode.js` | keep | 앵커 아홉 개 전부 그대로다. 0.86.0의 클립보드 계층 재작성은 `dist/utils/clipboard.js` 쪽이고, 우리가 쓰는 `utils/clipboard-image.js`의 `readClipboardImage`/`extensionForImageMimeType`은 그대로다. |

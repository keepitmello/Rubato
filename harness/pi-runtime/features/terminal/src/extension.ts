...
export interface TerminalExtensionHost {
	readonly createSettingsManager?: (cwd: string) => SettingsManager;
	readonly getShellEnv?: () => NodeJS.ProcessEnv;
}
...
function buildToolContext(pi: ExtensionAPI, state: TerminalExtensionState, host: TerminalExtensionHost): TerminalToolContext {
	...
	return {
		...
		getEnv: () => (host.getShellEnv ?? getShellEnv)(),
...
export function registerTerminalExtension(pi: ExtensionAPI, host: TerminalExtensionHost = {}): void {
	...
	const toolCtx = buildToolContext(pi, state, host);
	...
	pi.on("session_start", async (event, ctx) => {
		state.ctx = ctx;
		const settingsManager = host.createSettingsManager?.(ctx.cwd) ?? SettingsManager.create(ctx.cwd);

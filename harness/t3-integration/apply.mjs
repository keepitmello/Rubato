  ...
  'packages/tailscale/src/tailscale.ts': [
    ...
    ['export const disableTailscaleServe = (\n  input: {\n    readonly servePort?: number;\n  } = {},\n): Effect.Effect<void, TailscaleCommandError, ChildProcessSpawner.ChildProcessSpawner> =>\n  Effect.gen(function* () {\n    const servePort = input.servePort ?? DEFAULT_TAILSCALE_SERVE_PORT;\n    return yield* runTailscaleCommand(\n      ["serve", `--https=${servePort}`, "off"],\n      TAILSCALE_SERVE_TIMEOUT,\n    );\n  });\n\n',
      [
        ...
        '  Effect.gen(function* () {',
        '    const servePort = input.servePort ?? DEFAULT_TAILSCALE_SERVE_PORT;',
        '    // `tailscale serve --https=<port> off` clears the whole port mapping, not',
        '    // just the handler this process installed. A relaunch brings the incoming',
        '    // backend up before the outgoing one has torn down, so that late teardown',
        '    // would wipe a live mapping and leave the endpoint unreachable from the',
        '    // tailnet until someone turned it back on by hand. Clear it only while it',
        '    // is still ours.',

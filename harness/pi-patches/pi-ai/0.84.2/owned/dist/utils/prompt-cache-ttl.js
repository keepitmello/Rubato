...
function detectOpenAICompletionsCompat(model) {
    ...
    const supportsOpenRouterCacheControl = openRouterCacheControlPrefixes.some((prefix) => cacheControlModelId.startsWith(prefix));
...
export function supportsOneHourCacheTtl(model) {
    const candidates = getBedrockModelMatchCandidates(model.id, model.name);
    return candidates.some((candidate) => ["opus-4-5", "sonnet-4-5", "haiku-4-5"].some((modelVersion) => candidate.includes(modelVersion)));
}
/**

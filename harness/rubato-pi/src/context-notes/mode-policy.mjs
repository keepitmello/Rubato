...
// Every model starts in history-notes: notes and history are working memory for all
// models. How the window ends is a separate, per-model choice (context-budget.mjs):
// Astra/Grok roll over on notes, Claude keeps Anthropic server compaction, the rest
// only have a hard safety line. `summary` stays for sessions already recorded in it,
// an explicit RUBATO_CONTEXT_MODE=summary, and providers whose context is owned by an
// external executor (the notes controller cannot reset it).
export const SUMMARY_DEFAULT_PROVIDERS = Object.freeze(["claude-sdk-oauth"]);

const NOTES_WINDOW_TYPES = new Set([INIT_ENTRY, PREPARE_ENTRY, REMINDER_ENTRY, NUDGE_ENTRY]);
...
export function defaultContextModeForModel(model = {}) {
  return SUMMARY_DEFAULT_PROVIDERS.includes(model?.provider ?? "") ? SUMMARY_MODE : HISTORY_NOTES_MODE;
}

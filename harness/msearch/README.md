...
원본은 한 개인 동반 에이전트 시스템에서 채널 전용으로 자란 코드다. 여기서는 통로와
이름공간을 설정으로 빼서 아무 메모리 저장소나 가리킬 수 있게 하고, 그 시스템에만
있던 갈래(내면 큐·원문 전사·인물/정서 색인·에피소드 게이트)는 걷어냈다. 랭킹의
알맹이인 BM25 + 벡터 하이브리드는 그대로다.
...
| 변수 | 기본값 | 뜻 |
|------|--------|-----|
| `MSEARCH_ROOT` | `~/.rubato/memory/agents` | 색인 대상 루트 |
...
- 관련성 임계값을 못 넘으면 결과 대신 `NO RELEVANT MEMORY` 를 돌려준다. 낮은 점수의
  후보를 억지로 내놓는 대신, 없다고 말하고 호출자가 판단하게 둔다.

...
기억 저장소는 설정 없이 프로젝트마다 하나다. 폴더가 쓰는 저장소는 이 순서로 정한다.

1. 폴더 `.rubato/rubato.jsonc` 의 `memory.agent` — 여러 폴더를 한 저장소로 묶을 때 쓴다.
2. git 저장소 안이면 그 루트. `store.json` 의 `roots` 에 그 루트가 있는 저장소, 없으면 루트 이름
   (같은 이름을 다른 루트가 쓰고 있으면 짧은 해시를 붙인다).
3. 홈 디렉터리 그 자체면 `home`.
4. 그 밖에는 저장소가 없고, memory 도구는 기억이 꺼져 있다며 켜는 법을 알려 준다.

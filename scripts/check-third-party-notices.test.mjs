...
test("#given retained workspace notice requirements #when the requirement table is loaded #then every package path still exists", () => {
  for (const requirement of PACKAGE_NOTICE_REQUIREMENTS) {
    assert.equal(existsSync(join(repoRoot, requirement.path)), true, requirement.path)
  }
  assert.equal(BUNDLED_NOTICE_HEADINGS.includes("pi-lsp-client"), true)
  assert.equal(
    BUNDLED_NOTICE_HEADINGS.some((heading) => heading.includes("codegraph")),
    false,
  )
})

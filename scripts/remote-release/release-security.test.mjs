...
test("isolated uninstall revokes host Push state without touching global launchd or user prefixes", async (t) => {
  ...
  assert.equal(result.browserUnsubscribeRequired, false)
  assert.equal(result.browserCleanupCompleted, true)
  assert.match(result.browserCleanupInstructions, /PWA is discontinued/)
...
test("hub no longer ships Hono, web-push, or the terminal-bridge HTTP ticket path", async () => {
  ...
  assert.equal(rootPackage.overrides.hono, undefined)
  assert.equal(rootPackage.overrides["@hono/node-server"], undefined)
  assert.equal(hubPackage.dependencies.hono, undefined)
  assert.equal(hubPackage.dependencies["@hono/node-server"], undefined)
  assert.equal(hubPackage.dependencies["web-push"], undefined)
  assert.equal(hubPackage.dependencies.ws, undefined)
  assert.equal(hubPackage.dependencies["@rubato/terminal-bridge"], undefined)
  const hubLock = JSON.parse(await readFile(join(repository, "packages", "rubato-remote-hub", "package-lock.json"), "utf8"))
  assert.equal(hubLock.packages[""].dependencies.hono, undefined)
  assert.equal(hubLock.packages[""].dependencies["@hono/node-server"], undefined)
  assert.equal(hubLock.packages[""].dependencies["web-push"], undefined)
  assert.deepEqual(hubLock.packages[""].dependencies, { "@rubato/remote-protocol": "file:../rubato-remote-protocol" })
})

...
if [ -n "${RUBATO_CODEX_NODE:-}" ]; then
  if ! is_node_24 "$RUBATO_CODEX_NODE"; then
    echo "rubato-codex taskforce: RUBATO_CODEX_NODE must point to Node.js 24+" >&2
    exit 1
  fi
  ...
  if [ -z "$node_bin" ]; then
    echo "rubato-codex taskforce: Node.js 24+ is required; set RUBATO_CODEX_NODE" >&2
    exit 1
  fi
fi

exec "$node_bin" "$script_dir/../taskforce/dist/mcp-server.mjs"

...
A role requires a capability profile, not a permanent model name. The human chooses the lead model; the lead proposes workstream models and waits for approval. The bundled [model-guide](../../model-guide/SKILL.md) is the model and approval SSOT; [model allocation](model-allocation.md) keeps only the team-specific pointer.

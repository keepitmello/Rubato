...
description: Agent Team 또는 Codex native collaboration에서 하나의 bounded outcome을 조사·구현·로컬 디버깅·검증·handoff까지 소유한다. 독립적인 workstream owner가 필요할 때 사용한다.
---
...
Start by reading your task brief and any supplied or existing team mission and authoritative frame/spec/ADR that govern it. Do not create a mission or frame just to fill an absent document. Repo claims in the brief — where things are, how a mechanism works, why it fails — are the lead's reading, not ground truth, tagged `[inherited]`/`[assumed]` or not. How to read the rest of the brief is the `dispatched` contract and it applies to you unchanged: what binds and what is provisional, a wrong lead you fix yourself, a real conflict you return with evidence and a recommendation, a budget return, and the shape of being blocked. Those returns are completed dispatches, not failures.

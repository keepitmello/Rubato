...
이 규칙은 Codex가 즉흥 판단으로 엇나가지 못하게 하기 위한 **고삐**다. 위반 시 이전 test.md 세션처럼 "chrome 200에서 break → safari 미시도 → Playwright 미설치라 포기" 식의 오판이 재현된다.
...
**R3: 편향 금지**: `engine/**`, `waf_profiles.yaml`에 특정 사이트 도메인·셀렉터·브랜드명 하드코딩 금지. `python3 "${CODEX_HOME:-$HOME/.codex}/skills/insane-search/engine/bias_check.py"`가 CI 게이트. 자세한 규칙은 **No-Site-Name Rule** 섹션.
...
| `needs_js_exec` only | Playwright MCP (`an advertised Playwright/browser tool`) | Cloudflare 기본 방어 등 |
...
`fetch_chain`의 `needs_js_exec only` 케이스는 **Codex 세션에서 MCP 도구를 직접 호출**해야 한다. subprocess 경로 없음. 즉:
1. `result.summary`에 "Playwright MCP must be invoked from the Codex session"이 포함되면
2. 현재 런타임이 실제로 제공하는 브라우저 도구로 navigate → wait → snapshot 흐름으로 Codex가 직접 처리
...
```bash
cd "${CODEX_HOME:-$HOME/.codex}/skills/insane-search/engine/templates" && npm install
...
> 커버리지 회귀 점검: `python3 "${CODEX_HOME:-$HOME/.codex}/skills/insane-search/tests/coverage_battery.py"`로 플랫폼별 전수 경로 pass/fail + 썩은 예시 자동 적발.
...
이 섹션은 **참조 파일 선택 가이드**다. 문제가 생겼을 때 어떤 `references/*.md`를 열어야 할지 결정하는 기준으로 쓴다. Codex는 필요할 때만 해당 파일을 `Read`하고, 선제적으로 전부 읽지 않는다.
...
| [`playwright.md`](references/playwright.md) | engine이 Playwright fallback으로 넘어가는데 MCP/Local Chrome 중 어디로 갈지 확인 필요할 때 | Approach 1 (`an advertised Playwright/browser tool`: Cloudflare급 챌린지), Approach 2 (Local Node + `channel:'chrome'` + stealth: Akamai Bot Manager급), 템플릿 파라미터 규격 |

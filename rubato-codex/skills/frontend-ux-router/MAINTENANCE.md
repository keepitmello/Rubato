...
```bash
# 로컬이 source of truth. 수정은 항상 ${CODEX_HOME:-$HOME/.codex}/skills/frontend-ux-router/에서.
cd $(mktemp -d) && gh repo clone keepitmello/frontend-ux-router repo && cd repo
rsync -a --delete --exclude .git --exclude README.md ${CODEX_HOME:-$HOME/.codex}/skills/frontend-ux-router/ .
...
실물은 `${CODEX_HOME:-$HOME/.codex}/skills/frontend-ux-router/` 하나. 아래 4곳은 전부 여기로 걸린 심링크다.
...
아래 넷은 원래 `${CODEX_HOME:-$HOME/.codex}/skills/` 아래 독립 스킬로도 깔려 있었으나, 라우터를 통해서만 진입하면 되므로 2026-07-28에 독립 사본을 제거하고 이 안의 사본만 남겼다. 번들 사본은 upstream 원본이 아니라 **깨진 상호참조를 이 스킬 구조에 맞게 고친 판본**이다. upstream을 다시 당길 일이 있으면 링크 수정분이 날아가지 않게 diff부터 뜰 것.

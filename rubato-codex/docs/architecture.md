...
| 모델·추론 강도 선택 | 리드가 현재 노출된 GPT 선택지에서 배분 |

## 설치 경계

Codex 플러그인은 여섯 스킬과 MCP를 제공합니다. 전역 `AGENTS.md`와
...
모델이나 추론 강도를 고정하지 않습니다. 지원되는 네이티브 역할을 선택하면
...
- Rubato의 다중 프로바이더 model-guide와 공급자별 쿼터·폴백 정책.
- Rubato 고유 세션 목록·상태 UI·런타임 확장. 대응 기능은 Codex를 사용합니다.
- 역할마다 고정된 모델, 자동 모델 교체, 별도 스케줄러·상주 서비스.
- 보드 자체의 에이전트 실행·생존 판정·인증·원격 동기화.

현재는 같은 GPT 계열 안에서 작업별 모델을 고릅니다. 다른 GPT를 골랐다는
이유만으로 다른 모델 계열의 검증이라고 하지 않으며, 요청된 모델과 실제
런타임이 확인해 준 모델도 구분합니다. 다중 프로바이더 준비가 되면
`skills/agent-taskforce/references/model-allocation.md`와 런타임 어댑터를
확장하면 됩니다. 보드와 오너 계약은 그대로 유지할 수 있습니다.

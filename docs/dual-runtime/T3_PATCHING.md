...
있다고 표시되는 것이 아니다. 현재 핀의 추론 텍스트는 별도 `reasoning` 메시지로 저장되고,
일반 답변과 다른 행으로 그려진다. 타입뿐 아니라 저장 경로와 소비자까지 확인해야 한다.
...
재연결 직후는 세션이 아직 자기 자신을 모르는 구간이다. `get_state`가 이 대화의 모델이
아니라 카탈로그 기본 행을 답할 수 있고(272k 사건), 호스트 상태는 `refresh()`가 비동기라
실제보다 늦다. **그 구간에서 읽은 값을 그대로 내보내면 안 된다.**

뒤쪽이 실제 사고를 냈다. `agent_settled` 뒤에도 호스트는 잠시 `running`을 답하는데,
브릿지가 그걸 믿고 세션을 running 으로 두면 턴 id 는 이미 비어 있다. 그 상태에서 다음
프롬프트가 `prompt` 가 아니라 `steer` 로 나가고, 받아줄 턴이 없어서 영원히 안 끝난다.
되감기 직후 다음 메시지가 먹통이 되는 증상이 이것이었다. **실행 중 여부는 호스트가 아니라
우리 투영이 턴을 들고 있는지로 판단한다.**

시험에서는 이런 것이 "가끔 실패"로 보인다. 타임아웃을 늘려 덮지 않는다 — 깜빡이는 시험은
대개 제품 쪽 경합이고, 한 번 덮으면 아무도 다시 안 본다.
...
- **추론을 일반 답변으로 위장하기.** 현재 핀은 추론을 별도 `reasoning` 메시지로 저장한다.
  완료 아이템을 `assistant_message`로 잘못 보내면 추론이 답변에 섞이고 답변 구간까지
  끊는다. 시험도 “추론이 저장되지 않는다”가 아니라 “별도 역할로 저장되고 답변에 섞이지
  않는다”를 확인해야 한다.

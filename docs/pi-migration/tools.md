...
결론: stock Pi 0.85.1에 **일반 `executeTool` seam, Rubato `registerMcpServer` producer registry,
server별 direct/search 정책, owner-controlled lazy activation, session-owned MCP stdio client,
BM25 `tool_search` catalog**가 연결됐다. 현재 ast-grep producer는 기존 설정 그대로 eager/auto로,
memory는 기본 direct tool을 유지하고 `memory.tool_exposure: "search"`일 때만 lazy/search MCP로
들어간다. `tool_search`가 MCP owner callback으로 활성화한 직후 같은 host turn에서 일반
executor로 호출할 수 있으며 schema validation, tool hooks, progress, AbortSignal을 우회하지 않는다.
reload에서는 producer generation과 MCP service를 새로 만들고, shutdown에서는 subprocess와
서비스 소유 output artifact를 정리한다.
...
|---|---|---|---|
| MCP stdio lifecycle | MCP SDK `Client` + `StdioClientTransport`로 initialize 후 페이지별 `tools/list`; call은 timeout/signal/progress를 전달하고 `isError`를 실행 오류로 만든다. Senpi 기준은 `$SENPI/dist/core/extensions/builtin/mcp/expose/register.js:24-95`, pagination은 `.../pagination.js:1-28`, schema/result 변환은 `.../schema-compat.js:5-137`이다. | `harness/pi-runtime/features/mcp/service.mjs:35-431`의 session-owned service. enabled 서버만 시작하고 eager는 세션 시작 때 연결을 유지한다. cold lazy는 catalog discovery 뒤 닫고 첫 호출에 재연결하며 idle timeout을 적용한다. 시작 실패는 서버 이름이 든 오류로 거부하고 이미 열린 client를 rollback한다. | 현 slice는 current producer가 쓰는 stdio만 실행한다. HTTP/OAuth, proxy exposure, catalog disk cache, list-changed/resources/prompts는 지원한다고 가장하지 않고 visible unsupported error 또는 문서상 미완으로 남긴다. |
| MCP health/retry/output guard | Senpi는 30초 stale connection을 2초 ping하고 session-expiry와 retriable failed-send를 각각 한 번 재연결한다(`$SENPI/.../mcp/health.js:5-85`). 50KiB/2,000줄 초과 결과는 session owner의 0600 artifact로 spill하고 종료 때 지운다(`.../guard/output-guard.js:4-147`). | `service.mjs:108-164,338-429`가 call 때 mutable connection을 다시 읽어 session expiry/failed send를 1회 재시도하고 abort는 재시도하지 않는다. `output-guard.mjs:11-202`가 text/binary/resource payload, preview, mode 0600, owner cleanup을 보존한다. | background reconnect jitter/suspension state와 OAuth refresh는 아직 없다. `agentDir`가 주입되지 않으면 임의 profile에 쓰지 않고 inline truncation warning으로 안전하게 실패한다. |
| MCP identity/schema/result | 현재 Rubato wire name은 `mcp__<server>_<tool>`이며 선두 `_` server는 추가 `_`를 접는다. Rubato transform 근거는 `harness/rubato-pi/src/transforms/core-tool-surface.mjs:68-101`; Senpi의 64자 제한, `-`/`_` 충돌, SHA-1 4자리 suffix는 `$SENPI/.../mcp/expose/naming.js:1-55`다. | `features/mcp/compat.mjs:5-197`에서 같은 이름/충돌, local `$ref`, top-level `$schema`/`additionalProperties`, rich content/structured content를 보존한다. `same-name` fixture의 실제 suffix `5f5b`, `same_name`의 `f23a`를 고정했다. memory notice/nudge matchers도 이 wire name을 쓰되 bare `memory`/`memory_apply_patch` match를 유지한다(`packages/rubato-runtime/src/components/memory/tool-metadata.ts:5-13`, `nudge-wiring.ts:211-218`). | 결과 크기 제한과 session-owned artifact spill은 바로 위 output-guard 계약이 소유한다. 이 단계에서 이름을 stock Senpi의 `mcp_`로 되돌리면 Anthropic 경로와 기존 prompt/tool identity가 깨진다. |
| stock extension registration | stock 공개 API에는 dynamic `registerTool`, `exec`, active/all/set tools가 있다(`$STOCK/dist/core/extensions/types.d.ts:943-999`). 등록은 즉시 runtime refresh를 부른다(`.../loader.js:238-245`); AgentSession은 새 definition을 registry에 넣고 처음 본 extension tool을 active로 만든다(`$STOCK/dist/core/agent-session.js:2105-2180`). 장기 자원은 factory가 아니라 `session_start`에서 열고 `session_shutdown`에서 닫는 것이 공개 계약이다(`$STOCK/docs/extensions.md:220-224,1365-1373`). | `features/mcp/extension.mjs:7-84`가 lifecycle을 소유한다. 각 server policy에서 direct로 선택된 이름만 active set에 복원하고 search catalog identity는 MCP feed 하나가 소유한다. search 설정인데 `ToolSearchService`가 없으면 `MCP_TOOL_SEARCH_REQUIRED`로 실패한다. | SDK embedder는 정상 `bindExtensions` 경로를 사용해야 한다. stock core에 MCP 전용 API를 추가하지 않는다. |
...
| Rubato MCP producers | ast-grep은 `_ast_grep`, Node entry, project cwd env, enabled/eager/2초 startup을 선언한다(`packages/rubato-runtime/src/components/ast-grep/index.ts:13-51`). memory는 기본 direct tool이고 opt-in search일 때만 enabled MCP server를 선언한다(`packages/rubato-runtime/src/components/memory/tools.ts:142-176`). | `features/mcp-producers/registry.mjs:35-245`의 wrapper가 현재 Rubato factory에 nonoptional `registerMcpServer`를 주입하고 declaration을 normalize한다. 같은 owner의 reload generation은 교체하고 cross-owner duplicate, invalid declaration, missing Node entry는 visible error다. `createMcpExtension({servers: registry})`가 session start snapshot을 소비한다. | 기존 Senpi host의 optional missing-asset 정책을 바꾸지 않기 위해 ast-grep producer 원본은 수정하지 않았다. stock bootstrap은 bundle receipt/hash를 먼저 검증하고 registry도 실제 Node entry를 검사한다. caught/skipped required component를 성공 처리하지 않는 것은 bootstrap 조립 계약이다. |

## 구현된 공개 API와 조립점

```js
const registry = createMcpProducerRegistry({ registrationCwd: cwd });
const rubatoFactory = registry.wrapFactory(createRubatoComponentExtension(), {
  ownerId: "rubato-components",
  sourcePath: rubatoBundle,
  registrationCwd: cwd,
});
...
const extensions = [
  ...
  createMcpExtension({ servers: registry, agentDir, toolSearchService: search }),
];
...
- `createMcpProducerRegistry`는 current Rubato component factory의 declaration discovery owner다.
  `wrapFactory`를 거치지 않고 adapter API 부재를 기존 fallback으로 흘려보내면 stock candidate의
  필수 기능 누락이므로 성공으로 취급하지 않는다.
- `createMcpExtension`은 각 `session_start`에 service를 만들고 tool을 stock
  `pi.registerTool`로 등록한다. server별 `enabled`, `lifecycle`, `exposure`, `includeTools`,
  `excludeTools`, `directTools`를 적용하고 injected search service에 search catalog/activation owner를
...
- build descriptor는 `mcpFeature`(`features/mcp/feature.mjs`), `toolExecutionFeature`
  (`features/tool-execution/index.mjs`), `toolSearchFeature`(`features/tool-search/feature.mjs`),
  `mcpProducersFeature`(`features/mcp-producers/feature.mjs`)다.
...
11. 실제 current ast-grep producer → registry discovery → eager/auto direct call
12. memory search declaration shape → cold lazy discovery/close → search activation → reconnect/call
13. disabled server 무기동, invalid/duplicate/missing Node entry visible failure
14. session-expiry protocol error → 새 client/subprocess로 1회 retry, abort는 retry하지 않음
15. oversized result → agentDir의 mode-0600 spill → service shutdown artifact 삭제
16. `mcp__rubato-memory_*` result/call → receipt notice/provenance hook, 동시에 bare direct 이름 유지

소유 feature 집중 검증은 executor/MCP/producer/lazy-search 합계 `14/14`, all-selected composition
`1/1`이다. 실제 current component bundle은 빈 profile에서 direct와 search를 각각 실행해 search
catalog activation → `mcp__rubato-memory_memory` subprocess call → commit/RPC read-back까지 통과했다.
위 memory 훅 단위 테스트 4개는 이름 fixture를 함께 고정하지만, 이 격리 worktree에는 monorepo
workspace 의존성이 설치돼 있지 않아 독립 `bun test`는 import 단계에서 실행되지 않는다.
...
지금 완료된 것은 current Rubato producer가 선언한 **stdio MCP discovery + enabled/eager/lazy 및
direct/search/auto policy + local BM25 search/history + generic executor + health retry/output guard**다.
memory 기본 direct와 opt-in search 분기도 current component source에 그대로 남아 있고 stock wrapper는
search 분기에서만 MCP API를 제공한다. codemode도 staged source/assets와 실제 stock SDK의 JS
eval/tool/detach/shutdown까지 연결됐으며 자세한 미검증 경계는 [codemode.md](./codemode.md)가 정본이다.
MCP HTTP/OAuth, catalog disk cache, background reconnect backoff/suspension, proxy exposure, Anthropic native
search adapter, 전체 launcher/install/update는 각 소유 경계에서 아직 pending이다. 이 항목들이 연결되고 전체 E2E가 통과하기 전에는
